...
| Encode | `dns_encode` | [scapy/layers/dns.py:L154-L171] | Turns a dotted name into length‑prefixed labels (before any compression) |
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_upfront.py
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q1.py
...
**Function.** `dns_encode` [scapy/layers/dns.py:L154-L171]; the core transformation is the join at
L169.
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q2.py
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q3.py
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q4.py 2>&1
WARNING: DNS decompression loop detected
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q5.py 2>&1
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q6.py
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q7.py
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q8.py
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_q9.py 2>&1
WARNING: DNS decompression loop detected
INFO: DNS RR prematured end (ofs=243, len=2)
INFO: DNS incomplete jump token at (ofs=1)
INFO: DNS RR prematured end (ofs=6, len=3)
...
The very first dissection step is `DNS.pre_dissect` [scapy/layers/dns.py:L518-L538], which validates a
DNS‑over‑TCP length prefix before any field is parsed (for UDP it is a no‑op returning `s`
unchanged):
...
```text
$ PYTHONPATH=. /tmp/scapy-venv/bin/python /tmp/obs_extra.py 2>&1
...
| `dns_encode` | Q2 | [scapy/layers/dns.py:L154-L171] |
...
This investigation created and ran temporary observation scripts **outside** the repository, under
`/tmp`: `obs_upfront.py`, `obs_q1.py` … `obs_q9.py`, and `obs_extra.py` (plus a scratch notes file).
All were deleted after use. The only file added to the repository is this document. The captured
`git status` proof, taken from the repository root immediately before committing this file, is:
