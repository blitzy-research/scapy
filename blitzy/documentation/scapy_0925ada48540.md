...
- **Import method:** Scapy is installed **editable** (`pip install -e .`) into an isolated virtual environment, so `scapy.__file__` resolves to `<repo-root>/scapy/__init__.py` and `pip show scapy` reports `Editable project location: <repo-root>` — confirming the exercised code is the repository's own checked-out ISO-TP source, not a separately-packaged copy (see the Methodology note).
...
> **Methodology.** Evidence was captured in the **canonical configuration** a normal user would use: an **isolated virtual environment** with Scapy installed **editable** via `pip install -e .`, so the exercised code is the repository's own checked-out ISO-TP source (`pip show scapy` reports `Editable project location: <repo-root>`, and `scapy.__file__` resolves to `<repo-root>/scapy/__init__.py`). Each probe was run with that virtual environment's interpreter (written below simply as `python`), and the modules were exercised through their **real entry points** — `ISOTP.fragment()` (transmit segmentation) and `ISOTPSoftSocket` (receive reassembly). Thanks to the editable install, Scapy imports **without** any `PYTHONPATH`; `PYTHONPATH=<repo-root>` is additionally set for the receive probe (`/tmp/isotp_probe4.py`) **only**, so the **in-repo** `test.testsocket` package resolves (it is not part of the installed Scapy distribution — importing it without `PYTHONPATH` from outside the repository raises `ModuleNotFoundError`). `python-can` (a **test-only** dependency per `tox.ini:L33`, not a mandatory Scapy runtime dependency) was **not** required for the receive scenario: it was driven by the in-repo `test.testsocket.TestSocket(CAN).pair()` — the same mechanism the `.uts` unit-test suite uses. Temporary observation scripts lived under `/tmp/isotp_probe{1..4}.py` (outside the repository tree) and were deleted after use; the **tracked** `git status --porcelain` reports a clean tree (the only git-*ignored* byproducts present — the editable install's `scapy.egg-info/` and Python `__pycache__/` caches — are not tracked repository content; see the read-only evidence in Q8).
...
**Command** — `/tmp/isotp_probe1.py`, run with the editable-install venv interpreter as `python /tmp/isotp_probe1.py` (no `PYTHONPATH` needed):
...
**Command** — `/tmp/isotp_probe2.py`, run with the venv interpreter as `python /tmp/isotp_probe2.py` (no `PYTHONPATH` needed). It builds frames through the real entry point `ISOTP(data=b"\x00"*n).fragment()` for `n = 6, 7, 8, 20` and prints each frame's payload hex and first byte. It was **run twice — the counts are identical (stable)**.
...
**Command** — `/tmp/isotp_probe3.py`, run with the venv interpreter as `python /tmp/isotp_probe3.py` (no `PYTHONPATH` needed). It calls `ISOTP(data=b"\xaa"*n).fragment()` for `n = 4095, 4096, 5000`, prints the frame count and the FF bytes, prints the 5000-byte count twice (stability), and prints the sole `raise` statement in `fragment()`.
...
**Command** — `/tmp/isotp_probe4.py`, run with the venv interpreter as `PYTHONPATH=<repo-root> python /tmp/isotp_probe4.py` (the `PYTHONPATH` here makes the in-repo `test.testsocket` package importable). It pairs two in-memory sockets `cans, stim = TestSocket(CAN), TestSocket(CAN); cans.pair(stim)`, wraps one in `sock = ISOTPSoftSocket(cans, tx_id=0x641, rx_id=0x241)`, fragments a 20-byte payload `ff, cf1, cf2 = ISOTP(data=bytes(range(20)), rx_id=0x241).fragment()`, injects **FF** then **CF1** via `stim.send(...)` while **withholding CF2**, observes `sock.impl.rx_state` before/during/after, and measures the elapsed time to reset. It is run 3× — twice at the default `conf.verb=2` and once at `conf.verb=3` to unmask the warning.
