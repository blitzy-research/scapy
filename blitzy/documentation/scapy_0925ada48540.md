...
- In `rt2` the layer that actually splits off the trailing bytes is **`IP`** (above): IP's
  total-length field (`self.len`) covers the 20-byte `TCP` segment, so the 8 bytes *beyond* it are what
  become `Padding`. **`TCP` itself carries no length field and does not override `extract_padding`** — it
  inherits the base `Packet.extract_padding` returning `(s, None)` `[scapy/packet.py:982-991]`, so it
  splits off nothing (appending raw bytes directly after a bare `TCP` yields `Raw`, not `Padding`).
- `UDP` *is* a length-bearing transport layer (8-byte header with a `ShortField("len")`); its
  `extract_padding` splits at the UDP length:
...
claims nothing as padding, so for `Ether/Raw` there is simply no `Padding`. But length-bearing layers
(`IP`, `UDP`, `Dot3`) know their own length, so any bytes beyond that length are split off into a
`Padding` layer — in `rt2` it is `IP`, whose total-length field covers the `TCP` segment, that splits
the 8 trailing bytes, while `TCP` (no length field) splits nothing. And because
...
`b"\x00" * (conf.min_pkt_size - len(x))` `[scapy/arch/linux.py:570-573]` (full method L565-576); the
...
- **In length-bearing layers, at dissection time.** Layers that carry a length field override
  ...
  `[scapy/layers/inet.py:553-557]`, `UDP` at `[scapy/layers/inet.py:866-868]`, and `Dot3` (IEEE 802.3,
...
| **Dissection / parse** | `Ether(raw(pkt))` | **No** — dissection does not enforce a minimum | **Only for length-bearing layers** (IP/UDP/Dot3), and it then **persists** | `IP`/`UDP` `extract_padding` `[scapy/layers/inet.py:553-557, 866-868]`; `Dot3` `[scapy/layers/l2.py:275-302]`; `Padding(Raw)` re-emit `[scapy/packet.py:1906]` |
