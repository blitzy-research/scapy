...
**gateway's** MAC — not the external IP's. On the plain layer-3 send path, the **OS kernel** performs
that ARP after Scapy binds the socket to the interface.
...
**The L3 path is different.** If you send a plain `IP(...)` with no explicit `Ether()` layer, Scapy
does *not* run `getmacbyip`; instead `L3PacketSocket.send()` binds to the interface and lets the
**kernel** build the L2 header / issue ARP (`scapy/arch/linux.py:598-602`):

```python
# scapy/arch/linux.py:598
def send(self, x):
    # scapy/arch/linux.py:600
    iff = x.route()[0]
    if iff is None:
        # scapy/arch/linux.py:601-602
...
**Rationale.** Whether *Scapy* or the *kernel* issues the ARP depends on which layer you built: an
explicit `Ether()` makes Scapy resolve the MAC via `getmacbyip` (and thus apply the gateway swap),
while a bare L3 send delegates L2/ARP to the kernel after the socket binds to the chosen interface.
Either way, for an off-subnet target the link-layer destination is the **gateway**.
...
| **Q3 — External dest MAC** | Next hop is the **gateway** (`if gw != "0.0.0.0": ip = gw`), so ARP resolves the gateway's MAC; on L3 send the **kernel** ARPs after socket bind | `scapy/layers/l2.py:134-135`, `122-156`; `scapy/arch/linux.py:598-602` | who-has `pdst=10.236.12.1` (gateway), not `8.8.8.8` |
...
flowchart TD
    ...
    E --> G["iface.l3socket() -> L3PacketSocket.send<br/>bind to iface; kernel builds L2/ARP<br/>scapy/arch/linux.py:598-602"]
    F --> G
    G --> H{"explicit Ether()/L2 path?"}
    H -- "yes" --> I["getmacbyip(dst)<br/>scapy/layers/l2.py:122-156"]
    H -- "no (plain L3)" --> Z["kernel performs ARP"]
