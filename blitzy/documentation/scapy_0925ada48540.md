...
The `Ether.payload_guess` list contains **15 registered bindings** from the standard `scapy/layers/` modules (as of this commit). These are the bindings loaded by default via `from scapy.all import *` — 14 from `bind_layers()` calls plus 1 from `bind_bottom_up(Ether, EAPOL, type=0x888e)` in `scapy/layers/eap.py:852`:
...
flowchart TD
    ...
    B --> C["For each alias: iterate payload_guess<br/>(15 registered entries)"]
