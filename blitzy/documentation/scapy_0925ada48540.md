...
PYTHONPATH="$(git rev-parse --show-toplevel)" ./.venv/bin/python /tmp/scapy_investigation/probe.py > run1.txt
PYTHONPATH="$(git rev-parse --show-toplevel)" ./.venv/bin/python /tmp/scapy_investigation/probe.py > run2.txt
sha256sum run1.txt run2.txt   # identical hashes  => output is stable
diff run1.txt run2.txt        # empty             => byte-for-byte identical
...
```
6c2b053b6a5ed6eb18a9800abc20c8a210ee48f87e046a86f5aef3a8a6aee777  run1.txt
6c2b053b6a5ed6eb18a9800abc20c8a210ee48f87e046a86f5aef3a8a6aee777  run2.txt
```

**Repository left unchanged (OBSERVED).** The probe lives outside the checkout, so `git status --porcelain`
prints nothing — both while the probe exists and after it is deleted — and the only repository artifact is this
document:
