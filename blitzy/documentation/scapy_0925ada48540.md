...
> are pinned to Scapy `2026.06.26` running on Python `3.13.7`; the cited line numbers
...
### 1.2 How `load_contrib('isotp')` loads the module

`load_contrib('isotp')` resolves the module **by name** — it imports the package
`scapy.contrib.isotp` directly. Internally it calls
`importlib.import_module("scapy.contrib." + name)` and then
`_load("scapy.contrib." + name, ...)` [`scapy/main.py:L194-L214`, the import itself at
`L204-L205`]; if no contrib module of that name exists, it falls back to trying a *layer*
module of the same name [`scapy/main.py:L207-L213`]. In other words the string `'isotp'`
is simply appended to `scapy.contrib.` and imported — there is **no scan of the package's
contents** to decide what to load.

Two magic comments at the top of the package's `__init__.py` are easily mistaken for the
load mechanism, but they are **not** what `load_contrib` keys on
...
These two lines are **contrib-listing metadata**: they are parsed *only* by
`list_contrib()` [`scapy/main.py:L216-L269`, comment scan at `L258-L268`] to populate the
human-readable description/status that `list_contrib('isotp')` prints (here `status =
loads` denotes a contrib that loads without error). `load_contrib` itself never reads
them. After the import succeeds, the package re-exports its public symbols through
`__all__` [`scapy/contrib/isotp/__init__.py:L22-L25`]:
...
to idle and raises NO exception**. The partially received buffer is discarded and is
**never enqueued**, so **no completed message is ever delivered to the caller**. A
*bounded* receive — `sniff(timeout=…)`, a `select()`-based wait, or any receive path with
an external/socket timeout — therefore simply observes that **nothing arrived**. A **bare,
blocking `recv()`, however, does *not* return `None` when the timeout fires — it keeps
blocking** on the empty receive queue (see §8 step 3 below for the exact call chain). The
*only* diagnostic of the drop is a **verbose-only** log warning
...
rx_state reset to IDLE(0) after ~1.009s of missing CF
...
----------------------------------------------------------
direct blocking sock.recv(): STILL BLOCKING after 3.0s (did NOT return None at the cf_timeout)
DONE
```

(The leading `WARNING: RX state was reset due to timeout` line is the live verbose-only
log emission on **stderr**; every other line is the script's **stdout**. The final probe
runs a *bare* `sock.recv()` on a helper thread and waits 3 s — well past the 1 s
`cf_timeout` — and finds it **still blocked**, empirically confirming that a plain
blocking `recv()` does *not* return `None` after the timeout. The `message delivered to
rx_queue: None` line is a separate *non-blocking* check showing the queue stayed empty —
i.e. no completed message was delivered, which is a different fact from "recv returned
None".)
...
2. **No Consecutive Frame comes.** When the timer expires, `_rx_timer_handler` runs
   ...
   ~1.009s of missing CF` captures this.
3. **The caller is never handed a message — and a bare `recv()` *blocks*.** Because the
   transfer never completed, it is **never enqueued** into `rx_queue` (the completion path
   that would enqueue is `self.rx_queue.send(...)` at `isotp_soft_socket.py:L891`, and it
   is not reached). Tracing the receive call chain shows what a blocking `recv()` actually
   does: `ISOTPSoftSocket.recv` → `recv_raw` [`isotp_soft_socket.py:L172-L184`] →
   `self.impl.recv()` → `self.rx_queue.recv()` [`isotp_soft_socket.py:L976-L979`], and
   `rx_queue` is an `ObjectPipe` whose `recv()` performs `os.read(self.__rd, 1)`
   [`scapy/automaton.py:L196-L206`; the blocking read is `L202`]. That `os.read`
   **blocks until a byte is written** (which only happens when something is enqueued); the
   pipe returns `None` **only when it is closed and empty** [`scapy/automaton.py:L198-L201`].
   Consequently `recv_raw` yields `(basecls, None, None)` — and `recv()` returns `None` —
   **only if the socket/pipe is closed**, *not* on a CF timeout. With the partial message
   never enqueued and the socket still open, a **bare blocking `recv()` keeps blocking
   indefinitely**. (Note: the `recv_raw` docstring mentions a `self.timeout`, but the code
   path does not consult it — it calls `impl.recv()` unconditionally
   [`isotp_soft_socket.py:L172-L184`].) To observe the drop you must use a *bounded*
   receive — `sniff(timeout=…)`, `select()`, an external/socket timeout, or `close()`. The
   evidence lines `message delivered to rx_queue: None`, `exception raised? : NO`, and
   `direct blocking sock.recv(): STILL BLOCKING after 3.0s` capture exactly this.
...
surface a missing CF on their N_Cr timeout, but they do so by different means: the
pure-Python `python-can-isotp` library raises a `ConsecutiveFrameTimeoutError`, whereas the
Linux kernel `can-isotp` driver — being C, it cannot raise a Python exception — instead
reports a receive-data timeout *error code* such as `-ETIMEDOUT` to user space. Scapy's
`ISOTPSoftSocket` instead just resets state and (optionally) logs one line. If you rely on
an exception to detect a truncated transfer, **you will not get one here** — and you also
**cannot rely on a bare blocking `recv()` returning `None`**, because (as shown above) it
will simply keep blocking on the empty queue. Detect the drop yourself with a **bounded**
receive: pass a timeout to `sniff(timeout=…)`, use `select()`, set an external/socket
timeout, or `close()` the socket — then treat "no message within the window" as the
truncated-transfer signal.
...
|---|----------|--------------|--------------------------------------|
| **R1** | Set up Scapy & load ISO-TP | `load_contrib('isotp')` or `from scapy.contrib.isotp import ISOTP, ISOTPSocket`. ISO-TP is a **sibling** of `automotive`, not inside it. `ISOTPSocket` defaults to the pure-Python `ISOTPSoftSocket`. | `scapy/contrib/isotp/` package — `__init__.py:L6-L7`, `L22-L25`, `L27`, `L32-L47`; loader `main.py:L194-L214` |
...
| **R7** | Missing middle frame on receive | **`cf_timeout = 1 s`**, then **silent reset to idle, NO exception**. Partial message **never enqueued** → no message delivered; a bounded/`select`/`sniff(timeout=…)` receive sees nothing, while a **bare blocking `recv()` keeps blocking** (does *not* return `None`). Verbose-only warning `"RX state was reset due to timeout"`. | Experiment 2; `isotp_soft_socket.py:L497`, `L619-L629`, `L831`, `L842-L843`, `L172-L184`, `L976-L979`; `scapy/automaton.py:L196-L206` |
...
- `L172-L184` — `recv_raw` (calls `impl.recv()`; yields `(basecls, None, None)` **only** when `impl.recv()` returns `None`, i.e. when the rx pipe is closed) and the start of `recv`
...
- `L891` — `self.rx_queue.send(...)` (the completion enqueue — *not* reached on a dropped transfer)
- `L976-L979` — `impl.recv` (`return self.rx_queue.recv()`; blocks on the empty `ObjectPipe` — its docstring is literally "blocking if none is available in the buffer")

**`scapy/automaton.py`**
- `L196-L206` — `ObjectPipe.recv`: returns a queued item or `None` **only when the pipe is closed** (`L198-L201`); otherwise performs `os.read(self.__rd, 1)` (`L202`), which **blocks** until a byte is written on enqueue. This is why a bare blocking ISOTP `recv()` does *not* return `None` after a CF timeout.
...
- **Receiver-side N_Cr timeout** — the standard defines a timeout for the receiver
  awaiting Consecutive Frames. **Key implementation difference:** on expiry the pure-Python
  `python-can-isotp` library raises a `ConsecutiveFrameTimeoutError`, while the Linux kernel
  `can-isotp` driver reports a receive-data timeout *error code* such as `-ETIMEDOUT` (it
  does not raise a Python exception), **whereas Scapy's `ISOTPSoftSocket` silently resets its
  receive state and raises no exception** (R7).

### 10.3 Environment & reproduction notes

- **Pinned environment:** Scapy `2026.06.26`, Python `3.13.7`. The cited line numbers
...
- **Reproducing Experiment 2 (receive timeout):** set `conf.verb = 3`; create a paired
  ...
  exception and an empty `rx_queue`. To confirm the blocking-`recv()` behaviour, run a
  *bare* `sock.recv()` on a helper thread and `join(timeout=3.0)`: the thread is **still
  alive** afterwards, showing `recv()` keeps blocking rather than returning `None`. (Timing
  values such as `+0.005s` / `~1.009s` are inherently per-run and will vary slightly
  between executions.)
