...
reference), not from the dissection engine. In the Q1–Q5 sections below, stderr is redirected
to a separate stream so each **stdout** block shows the dissection output cleanly; this warning
is the *only* content that appeared on stderr for those scripts. (For Q6 the stderr is shown,
because it also carries the `log_runtime.error` lines that are part of that demonstration.)
...
Output (complete, unedited — including every `= None` HTTP header line):
...
Output (the `.show()` is the misparse proof; the last line is the control):
...
[`scapy/layers/http.py:637`] — *could* catch such a mismatch; see Synthesis.)
...
Log output (stderr — the two leading lines are the import caveat above, not a result; the `ERROR:` lines are `log_runtime.error` from the handler):
...
- `debug_dissector` **nonzero** (`1` and `2` behave **identically** here): a `log_runtime.error`
  ...
  (Case B), it logs `... returned [None]` and still falls back to `Raw` (no raise). The
...
| **TCP** | Q1 (`dport=80 → HTTP`), Q2 (`dport=40001 → Raw`) | `TCP` `inet.py:753`; `bind_bottom_up(TCP,HTTP,...)` `http.py:748`–`750` |
