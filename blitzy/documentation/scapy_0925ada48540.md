...
**Evidence — the folded outer address is the `caa80165` / `caa80166` prefix (A1); the in-repo assertion is replayed verbatim in C5.**
...
**Evidence — A4 (checkIPinIP=True → outer prefix retained → NO collision) vs A5 (checkIPinIP=False → outer folds away → COLLISION), and the end-to-end B-series:**
...
**Bottom line for M5:** the ICMP-error wrong-match is *not* a distinct `Raw` mechanism; it is mechanism M4 (`checkIPinIP=False`) acting on an error that quotes the decapsulated inner packet with identical inner content. A unique inner sequence is a *working* discriminator when it is present; it fails to help only when the collision is forced at the outer-IP layer (which the inner sequence cannot influence) or when the sequence is truncated away (leaving the error unmatched, not mis-matched).
...
| **unique ICMP sequence number** | Included by `ICMP.hashret` only for echo-family types in `icmp_id_seq_types` [scapy/layers/inet.py:L949, scapy/layers/inet.py:L986-L989]. It **is** a real discriminator when it survives: it separates echo probes (A2) and correctly picks the right probe inside an ICMP-error citation that preserves it (D3). It does **not** explain the reported wrong-match, because the collision is forced at the *outer-IP* layer (M3/M4), which the inner sequence cannot influence; and if truncation strips it, the probe is left **unmatched** (D4), not wrong-matched. | A2, D3, D4 |
...
- *Unique ICMP sequence* — a *working* discriminator wherever it survives (A2, D3); it cannot fix a collision created at the outer-IP layer, and truncation that strips it yields an *unmatched* error (D4), not a wrong one.
...
```bash
# 1) Private working dir OUTSIDE the repo (fails if it can't be created uniquely; mode 0700),
#    with an EXIT trap that removes ONLY the four known files and then rmdir's the directory.
REPRO="$(mktemp -d "${TMPDIR:-/tmp}/repro.XXXXXX")"
chmod 700 "$REPRO"
trap 'rm -f "$REPRO"/repro_matching.py "$REPRO"/repro_sndrcv.py \
              "$REPRO"/repro_error.py "$REPRO"/repro_controls.py; \
      rmdir "$REPRO" 2>/dev/null || true' EXIT
...
#   C5  VERBATIM replay of test/regression.uts '= answers' (L1212/L1214-L1233), INCLUDING the
...
# C5: VERBATIM replay of test/regression.uts '= answers' (L1214-L1233), including L1220-L1222
reset()
...
print("[C5] test/regression.uts '= answers' (L1214-L1233) replayed VERBATIM incl. L1220-L1222 -> ALL PASSED")
...
[C5] test/regression.uts '= answers' (L1214-L1233) replayed VERBATIM incl. L1220-L1222 -> ALL PASSED
[C6] (checkIPsrc, checkIPaddr) end-to-end truth table (non-tunnel echo, identical seq, reply from gw2)
     checkIPsrc  checkIPaddr  collide?  pairing
     True        True        False     CORRECT(gw2)
     True        False       True      WRONG(gw1)
