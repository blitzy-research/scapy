...
- **Environment-specific vs. general.** Two reported results are host/environment-specific and are labeled as such where they appear: **(a)** the version *digits* (`2026.07.08`) are derived from a file modification time, and **(b)** the socket classes are selected for **Linux**. Everything else (verbosity default, layer counts, packet structure, theme name, code paths) generalizes across platforms.
...
Scapy prints a **two-column banner**: a **green ASCII-art logo** on the left beside a **blue info column** on the right that reads `Welcome to Scapy` / `Version <YYYY.MM.DD>` / `https://github.com/secdev/scapy` / `Have fun!` / `<random quote>`. In this build the version line is **`Version 2026.07.08`**. The banner (and the version string) are written to **stderr**, preceded by a few `INFO`/`WARNING` lines emitted by optional extras. The version is **not a real release number** — it is the **file-modification-time fallback** of `_version()`, because this checkout has no `SCAPY_VERSION`, no `VERSION` file, no git-archive metadata, and **no git tags**.
...
**Version derivation trace** (a temporary `/tmp` script that calls each `_version()` method directly):

```
$ PYTHONPATH=. python3 /tmp/obs_version_trace.py
=== Observed VERSION / __version__ ===
scapy.VERSION      = 2026.07.08
scapy.__version__  = 2026.07.08

=== Method 0: SCAPY_VERSION env var ===
'SCAPY_VERSION' in os.environ = False

=== Method 1: scapy/VERSION file ===
VERSION file path = /tmp/blitzy/scapy/blitzy-d739fed8-298d-4380-ace7-4acedb1d7062_0057fd/scapy/VERSION
exists = False

=== Method 2: _version_from_git_archive() ===
raised ValueError: not a git archive

=== Method 3: _version_from_git_describe() ===
raised ValueError: tag has invalid format

=== Inside Method 3: raw `git describe --tags --always --long` ===
rc=0 stdout='0925ada4' stderr=''

=== _parse_tag on the bare SHA (should raise) ===
raised ValueError: tag has invalid format

=== Fallback: mtime of scapy/__init__.py ===
...
flowchart TD
    ...
    M3["Method 3: _version_from_git_describe() (L153)"] -->|"git describe -> bare SHA '0925ada4' (0 tags); _parse_tag raises ValueError: tag has invalid format (L35)"| FB
...
**Command** (run inside the real console; a `/tmp` script of `print(...)` statements is concatenated with `exit()` and piped to `python3 -m scapy`; stderr/banner suppressed with `2>/dev/null`; the `>>> ` prompt echoes are stripped):

```
$ cat /tmp/obs_q2.py - <<'CMDS' | PYTHONPATH=. python3 -m scapy 2>/dev/null
exit()
CMDS
```

where `/tmp/obs_q2.py` contained:
...
**Output** (ANSI-stripped; stable across 2 runs):
...
**Command** (inside the real console, ANSI stripped, prompt echoes removed):

```
$ cat /tmp/obs_q3.py - <<'CMDS' | PYTHONPATH=. python3 -m scapy 2>/dev/null
exit()
CMDS
```

where `/tmp/obs_q3.py` contained:
...
**Command** (structure inspection, inside the real console, ANSI stripped, prompt echoes removed):

```
$ cat /tmp/obs_q4.py - <<'CMDS' | PYTHONPATH=. python3 -m scapy 2>/dev/null
exit()
CMDS
```

where `/tmp/obs_q4.py` contained:
...
**The `.show()` dump** (`pkt.show()` on the same object; ANSI-stripped):
...
**Command — console** (inside `python3 -m scapy`, ANSI stripped, prompt echoes removed):

```
$ cat /tmp/obs_q5.py - <<'CMDS' | PYTHONPATH=. python3 -m scapy 2>/dev/null
exit()
CMDS
```

where `/tmp/obs_q5.py` contained `print("console_theme:", type(conf.color_theme).__name__)` (plus an `isinstance` check against `AnsiColorTheme`).

**Command — plain import** (NOT the console):

```
$ PYTHONPATH=. python3 -c "from scapy.all import conf; print('plain_import_theme:', type(conf.color_theme).__name__)"
```

**Output** (both, ANSI-stripped; stable across 2 runs):
...
- **Generalizes across platforms and runs** (deterministic, code-driven):
  ...
  - **Q2 counts** `len(conf.load_layers) = 49` and `len(conf.layers) = 1319` (fixed for this source tree), and the "files ≠ loaded" reasoning.
