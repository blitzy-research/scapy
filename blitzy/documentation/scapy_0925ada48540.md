...
```
`[scapy/layers/dns.py:L103]`

This is a bitmask test, **not** an equality test. The two complementary branches sit just below it: an ordinary label is handled by `elif cur > 0:` `[scapy/layers/dns.py:L132-L135]`, and a zero length octet terminates the name in the `else: break` `[scapy/layers/dns.py:L136-L137]`.
...
**Code excerpt** — `decodeRR` threads the entire packet body into every record via `_orig_s=s`:
...
The complementary action — switching the working buffer to that full body when a pointer must be followed — happens inside `dns_get_str`: it saves the current record's continuation with `bytes_left = s[after_pointer:]`, sets `s = s_full`, recomputes `max_length`, and flips `_fullpacket = True` `[scapy/layers/dns.py:L118-L125]`.
...
**Code excerpt** — the suffix enumeration in `possible_shortens` (full name first, then each successive dot-suffix):
...
```
`[scapy/layers/dns.py:L213-L215]`

The enclosing section walk visits the four sections in the fixed order `qd → an → ns → ar` via `for lay in [dns_pkt.qd, dns_pkt.an, dns_pkt.ns, dns_pkt.ar]:` `[scapy/layers/dns.py:L196]`.
...
**Code excerpt** — the decoded name is built purely from the labels actually reached:
...
A zero length octet ends the name through the `else: break` `[scapy/layers/dns.py:L136-L137]`, so only the reached labels contribute to the result — the output depends on *which labels are reached*, not on how the encoder placed the pointers.
...
**Code excerpt.** The dissection driver that turns each record's raw bytes into a resolved name is the single call inside `DNSRRField.getfield`:

```python
name, p, _, _ = dns_get_str(s, p, _fullpacket=True)
```
`[scapy/layers/dns.py:L387]`

The build side is the equally compact opt-in one-liner `return dns_compress(self)` `[scapy/layers/dns.py:L514-L516]`.

**Rationale.** The end-to-end answer is split into **dissection**, **build**, and **twisted** paths because those are three *independent* entry points in Scapy's runtime control flow, not three stages of one linear pipeline. Dissection is driven automatically by the field machinery — `DNSRRField.getfield` calls `dns_get_str(..., _fullpacket=True)` to resolve each name as the bytes are parsed `[scapy/layers/dns.py:L375-L396]` — so name resolution happens unconditionally whenever a packet is dissected. Build is a deliberately *decoupled, opt-in* path: `post_build` only frames the TCP length and never compresses `[scapy/layers/dns.py:L509-L512]`, and compression occurs solely when the caller explicitly invokes `compress()` → `dns_compress(self)` `[scapy/layers/dns.py:L514-L516]`. Twisted-packet handling is not a fourth pipeline but the malformed-input behavior *inside* the dissection loop (the Q4/Q5 guards), surfaced separately because a robustness account must state exactly when Scapy degrades gracefully (warn/info + `break`, slice-clamp) versus when it raises (`Scapy_Exception`). Presenting the three separately mirrors how the code is actually organized — **decode-on-parse**, **compress-on-request**, and **defensive guards within the decode loop** — and lets each claim be cited to the function that governs it.
