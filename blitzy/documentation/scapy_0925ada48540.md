...
Each question opens with a short **Direct answer** that *summarizes* the finding; every individual claim in it is then restated and tagged (`[observed]` / `[inferred]`) beside its evidence in the subsection that follows, so the Direct-answer summaries themselves are left untagged by design.
...
- **Q4, Q3, and Q2** are all governed by the **`raw_packet_cache` mechanism** that is populated *only when a packet is dissected from bytes*. The document establishes that mechanism first (Q4) and derives Q3 and Q2 from it (Q3 → Q2).
...
- **Scope of the version claim.** The mechanism is implemented in pure-Python control flow (dict/`list`/`bytes` comparisons in `self_build`/`do_dissect`), so it is *expected* to behave identically on the repository's full supported range (`pyproject.toml`: `requires-python = ">=3.7, <4"`). That broad-range equivalence is **[inferred]** from the source, not exhaustively tested; the only interpreters empirically verified here are **3.11.13 and 3.13.7**. **[inferred]**
- **Zero mandatory dependencies** — Scapy's core is dependency-free; `cryptography` is only an optional extra, unrelated to this mechanism. **[inferred]** (`pyproject.toml`).
...
- When both caches are present `[scapy/packet.py:685-686]`, it recomputes each snapshotted field's **live** value with the same `_raw_packet_cache_field_value()` and compares to the stored snapshot `[scapy/packet.py:687-689]`. **[inferred]** (source). The Q3 reproduction (below) corroborates both outcomes of this comparison. **[observed]**
...
Three of the four questions share **one** mechanism: the `raw_packet_cache` byte-cache that `do_dissect()` populates and that `self_build()`/`do_build()` consult. **Q1 is different** — it concerns `post_build` field-calculation *ordering* on a freshly-constructed (uncached) packet, and only touches the cache *after* a re-dissection, where the cache then preserves the already-computed value.
...
Every named entity and question part from the prompt, with where it is addressed and its evidence label (**[observed]** = seen in an entry-point run above; **[inferred]** = read from source). The per-question "Direct answer" paragraphs are intentional prose summaries and are substantiated by the labeled evidence that follows each of them.
...
- [x] `raw()` / `bytes_encode()` entry points — exercised throughout (Q1/Q4) **[observed]**; `[scapy/compat.py:112, 121]` **[inferred]** (source).
