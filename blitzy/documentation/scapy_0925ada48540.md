...
While debugging a custom protocol layer whose checksum depends on a length field that
in turn depends on payload size, four puzzling behaviors were observed. This document
diagnoses each one **from the source code as the single source of truth** and backs every
conclusion with a **runnable reproduction and its verbatim observed output**. The four
questions are:

- **Q1 — Deferred field calculation & the "`show2()` twice" anomaly.** Building a stack
  like `MyProto()/SomePayload()` computes the checksum *before* the length field is
  finalized, yielding a wrong checksum on the first serialization; calling `show2()` a
  second time can show the correct checksum. Why is the build non-idempotent?
- **Q2 — The "dual-state" packet after dissection.** A packet parsed via
  `MyPacket(raw_bytes)`, after editing the payload of a sub-packet inside a
  `PacketListField`, returns the *original* bytes from `bytes()` while `show()` displays
  the *modified* value — two states at once. Can this be verified, and does `copy()` fix it?
- **Q3 — Direct-field vs. nested-payload modification.** Why does modifying a field on the
  *same layer* that owns the cached bytes trigger a correct rebuild, while modifying a
  field on a *nested sub-packet* does not?
- **Q4 — The complete lifecycle & invalidation mechanism.** What exactly is cached during
  parsing, what triggers cache invalidation, and why are nested modifications invisible to
  the rebuild process?

**Provenance.** All citations refer to the Scapy source at commit
`0925ada4` ("Add AUTOSAR PDUTransport/PDU with initial batch of tests (#3933)"). Source
locators are written as `file:Lnnn` or `file:Lnnn-Lmmm`. Reproductions were executed with
**Python 3.11.15** and **Scapy `VERSION 2026.06.26`**, with Scapy imported directly from
the repository source tree via `PYTHONPATH` (never `pip install`, so no packaging artifacts
are written into the read-only repository). The scripts live outside the repository under
`/tmp` and are removed when the investigation concludes — they are never committed.

> **A note on the runtime.** The project declares `requires-python = ">=3.7, <4"`
> (`pyproject.toml`), and its highest *documented*/CI runtime is Python 3.11 (`tox.ini`
> `envlist` up to `py311`). The reproductions here ran on Python 3.11.15, which is within
> that range and matches the project's highest documented version. The earlier project
> analysis referenced Python 3.12.3; both runtimes produce **byte-for-byte identical
> output** because the analyzed build/dissect/cache mechanism is pure-Python and
> version-agnostic across these releases. Observing the same hex on two different Python
> minor versions is itself corroboration of that version-independence.
...
documentation describes the field-level versions of these as the *human*, *internal*, and
*machine* representations of a field value (`doc/scapy/build_dissect.rst:L151`, `:L254`,
`:L264`). Lifting that to the whole packet, the three representations relevant to this
analysis are:
...
1. **The assembled/dissected object** — a live `Packet` instance: a tree of layers where
   ...
   `show()` walks and what you mutate when you write `pkt.subs[0].payload.x = 0x42`.
2. **The internal/field values** — the Python values held in each layer's `.fields` dict
   (and resolved through `getfieldval`). A field left unset is stored as `None` and is
   *resolved* (to a default, an auto-computed length, etc.) only at build time.
...
- **`explicit`** — a flag that is `0` for a freshly constructed packet and `1` after
  dissection (`scapy/packet.py:L1020`). It is the gate inside `do_build()`
...
`Packet_metaclass` (`scapy/base_classes.py:L281`, field resolution at `:L288` onward;
`BasePacket` at `:L510`-region, defined at `:L419`). Field *order* therefore determines the
...
(`doc/scapy/build_dissect.rst:L546`, `:L556`, `:L623-L658`).
...
"second call is right" experience. The documentation describes `show2()` as the method that
shows the packet *"so that automatic fields are calculated (checksums, etc.)"*
(`doc/scapy/build_dissect.rst:L257`, `:L262`).
...
assignment routes through `__setattr__` → `setfieldval()` (`scapy/packet.py:L494`, `:L472`),
...
`self.raw_packet_cache_fields` (`scapy/packet.py:L1005`, populated at `:L1015-L1017`). The
...
- **`show()` walks the live object tree.** `show()` renders the in-memory `Packet` tree
  field by field, which already reflects your edit. It does not consult `raw_packet_cache`.
...
[Section 7, Script B](#script-b--tmpq_testpy-q2q3q4-dual-state--copy--clear_cache--direct-edit)
...
(`PacketListField` assembly at `:L411-L427`; *"Nested PacketListField"* at `:L443`, `:L514`,
`:L611`), confirming that a `PacketListField` of sub-packets that themselves carry a payload
...
at `scapy/packet.py:L1057`), **every sub-layer gets its own cache too**. The regression test
...
|---|---|---|
| `pkt.field = v` → `__setattr__` → `setfieldval` | Nulls `raw_packet_cache`, `raw_packet_cache_fields`, `wirelen` on the **owning** layer and sets `explicit = 0`; for a non-owned attr recurses **down the payload only** | `scapy/packet.py:L494`, `:L472`, body `:L482-L492` |
| `del pkt.field` → `__delattr__` → `delfieldval` | Same nulling + `explicit = 0`; this is why deleting an auto field forces recomputation | `scapy/packet.py:L519`, `:L504-L511` |
...
| implicit, inside `self_build` | If a snapshotted field no longer matches its dissect-time value, the cache is nulled and a real build proceeds | `scapy/packet.py:L685-L695` (null at `:L690-L693`) |
...
- **Use a length field that resolves during `self_build`.** Prefer a `FieldLenField` (or
  similar auto-length field) for the length. Such fields resolve their value while
  `self_build` walks `fields_desc` (`scapy/packet.py:L696-L714`) — i.e. *before* `post_build`
  runs (`scapy/packet.py:L758`) — so the checksum computed in `post_build` already sees a
  finalized length.
...
output is produced on Python 3.12.3; the mechanism is pure-Python and version-agnostic.) The
output shown is the **verbatim** observed output.
...
| `do_build()` and the `explicit` gate `if not self.explicit: self = next(iter(self))` | `scapy/packet.py:L724`, gate at `:L731-L732` |
...
| `self_build()` field-order serialization | `scapy/packet.py:L678`, field loop `:L696-L714` |
| `self_build()` cache short-circuit (compare snapshot, return cached) | `scapy/packet.py:L685-L695` (null on mismatch `:L690-L693`) |
| `post_build()` hook (late-evaluated fields) | `scapy/packet.py:L758-L770` |
| snapshot granularity `[x.fields for x in val]` (top-level only) | `scapy/packet.py:L648`, list case `:L652-L661` |
| `copy()` preserves cache + `explicit` | `scapy/packet.py:L407`, preserve at `:L415-L419` |
| `clear_cache()` recursively nulls cache (sub-packets + payload) | `scapy/packet.py:L664-L676` |
| `setfieldval()` nulls cache, resets `explicit = 0`, recurses down payload only | `scapy/packet.py:L472`, body `:L482-L492` (`explicit = 0` at `:L484`) |
...
| `do_dissect()` sets cache + snapshot + `explicit = 1` | `scapy/packet.py:L1002`, snapshot `:L1005`/`:L1015-L1017`, cache `:L1019`, `explicit = 1` at `:L1020` |
...
| `show2()` = `self.__class__(raw(self)).show(...)` | `scapy/packet.py:L1473`, body `:L1486` |
| `raw()` returns `bytes(x)`; `bytes_encode` | `scapy/compat.py:L112`; `:L121` |
| `Field.do_copy` (list/packet snapshot copy) | `scapy/fields.py:L256-L266` |
| `_PacketField` / `PacketField` / `PacketListField` / `PacketListField.getfield` | `scapy/fields.py:L1473` / `:L1525` / `:L1562` / `:L1721` |
| `Packet` metaclass establishes `fields_desc` ordering | `scapy/base_classes.py:L281` (resolution `:L288`+); `BasePacket` `:L419` |
| official build/dissect, `show2`, `post_build` docs; "dissecting is building" duality | `doc/scapy/build_dissect.rst` (`show2` `:L257`/`:L262`; `post_build` `:L546`/`:L556`/`:L623`/`:L632`/`:L649`/`:L655`/`:L658`; dissect pipeline `:L285-L296`; duality `:L333-L348`) |
| `.raw_packet_cache` regression assertion (`is not None` after dissection) | `test/regression.uts:L3974` (header), `:L3978` (assertion) |
| nested `PacketListField` scenario | `test/fields.uts` (`PacketListField` `:L411-L427`; "Nested PacketListField" `:L443`/`:L514`/`:L611`) |
