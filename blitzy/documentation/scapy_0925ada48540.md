...
> **Note on the interpreter version.** The observed interpreter is **CPython 3.13.7** — the only
> interpreter available in the canonical container. Per the governing "report exactly what you
> observe" rule, this document reports `3.13.7`. The DNS codec is pure‑stdlib and its behavior is
> interpreter‑agnostic across supported Python 3 versions, so every byte string below is stable
...
`scapy/error.py` installs a `logging.StreamHandler` (`scapy/error.py:L115`) whose formatter format
string is `"%(levelname)s: %(message)s"` (`scapy/error.py:L118`). The master `scapy` logger defaults
...
- `17` — the end offset: the 16 input bytes were fully consumed and `pointer` sits one past them.
...
- **Premature end** (truncated data, or a pointer to a nonexistent/negative offset) →
  `INFO: DNS RR prematured end (ofs=..., len=...)` (SQ4a), then returns the partial name.
- **Incomplete jump** (truncated mid‑pointer) → `INFO: DNS incomplete jump token at (ofs=...)`
  (SQ4b), then returns the partial name.
...
|------|---------|-------------|-------------------|------------------|---------------|
| **SQ1** decompression entry | `dns_get_str(...)` | `scapy/layers/dns.py:L69` | `(b'www.example.com.', 17, b'', False)` | reached from `DNSRRField.getfield` L387 & `DNSStrField.getfield` L305 | Reads length byte, copies label, appends dot, loops until `0x00` |
...
| return tuple | `return name, pointer, bytes_left, len(processed_pointers) != 0` | `scapy/layers/dns.py:L144` | `(..., 17, b'', False)` | 4th elem `True` when a pointer was followed | Reports name, end offset, remainder, used‑pointer flag |
| **SQ2** encoder | `dns_encode(...)` | `scapy/layers/dns.py:L154` | `dns_encode(b"www.example.com") -> b'\x03www\x07example\x03com\x00'` | `www.google.com`→`\x06google`; `*`→`b'\x01*\x00'` | Length‑prefixed labels + root octet |
| label build / 63‑cap | `... (k[:63] for k in x.split(b"."))` | `scapy/layers/dns.py:L169` | `70×'a'`: `first_len_byte=63, kept_bytes=63` | — | Labels capped at 63 bytes |
| root terminator | `if x[-1:] != b"\x00": x += b"\x00"` | `scapy/layers/dns.py:L170-171` | `raw(DNS()) -> b'...\x03com\x00\x00\x01\x00\x01'` | — | Appends `\x00` only if missing |
...
| loop detect + warn | `if pointer in processed_pointers:` / `warning(...)` / `break` | `scapy/layers/dns.py:L115-117` | `(b'data.data.', 7, b'', True)` | corpus L153 `== b"data.data."` | Target checked *after* first jump ⇒ one extra label |
| **SQ4a** premature end | `if abs(pointer) >= max_length:` / `log_runtime.info("DNS RR prematured end ...")` | `scapy/layers/dns.py:L96-99` | `INFO: DNS RR prematured end (ofs=6, len=1)` | `b"\x06da"`→`ofs=7`; `b"\x04data\xc0\x01"`→`ofs=-11` | Truncation / negative offset caught by `abs()` |
| **SQ4b** incomplete jump | `if pointer >= max_length:` / `log_runtime.info("DNS incomplete jump token ...")` | `scapy/layers/dns.py:L108-111` | `INFO: DNS incomplete jump token at (ofs=1)` | fires *before* no‑context check L118 | Second pointer octet missing |
| **SQ4c** no‑context raise | `raise Scapy_Exception("DNS message can't be compressed ...")` | `scapy/layers/dns.py:L128-129` | `Scapy_Exception: DNS message can't be compressed at this point!` | vs. graceful log‑and‑break in SQ4a/b/SQ3 | Pointer seen but no buffer to resolve |
...
| record walk order | `[dns_pkt.qd, dns_pkt.an, dns_pkt.ns, dns_pkt.ar]` | `scapy/layers/dns.py:L196` | SOA `ns.rrname` → `b'\xc0\x10'` | MultipleTypeField types `[2,3,4,5,12,15]` L205 | Fixed qd→an→ns→ar order ⇒ deterministic target |
...
| pointer math | `fb_index = ((index >> 8) \| 0xc0)`; `sb_index = ...`; `pointer = chb+chb` | `scapy/layers/dns.py:L225-229` | close‑index `\xc0\x0c` (corpus L214) | — | Packs 14‑bit offset + `0xc0` marker |
...
| **SQ7** `_is_ptr` | `b"." not in x and (...)` | `scapy/layers/dns.py:L147-151` | `_is_ptr(b"\x01*\x00")=True`; `_is_ptr(b"www.example.com.")=False` | `_is_ptr(b"\xc0\x0c")=True` | No dot + ends `0x00` or `0xc0` high bits ⇒ built |
...
| determinism round‑trip | `raw(DNS(raw(cp[DNS]))) == raw(cp[DNS])` | `test/scapy/layers/dns.uts:L128` | `-> True` | MX frame L139 `raw(dns_compress(pkt)) == frame` | Same bytes dereferenced + deterministic search |
...
| **UE3** truncated | prematured end / incomplete jump | `scapy/layers/dns.py:L96-99,L108-111` | `ofs=6, len=1` / `ofs=1` | (mid‑label vs mid‑pointer) | Guards catch missing bytes |
| **UE4** OOB / no‑context | jump math + `abs` guard / raise | `scapy/layers/dns.py:L114,L96-99,L128-129` | `ofs=-11` / `Scapy_Exception` | graceful vs. raise | Negative offset caught; no‑context raises |
