...
| `test/scapy/layers/http.uts` | 8 | 1 | Missing optional library (brotli) |
...
**Failure context:** The 63 failures fall into three categories:
1. **Missing external tool dependencies (tcpdump/tshark)** — 16 failures across `regression.uts` (12 of its 14 failures) and `gxrp.uts` (2 failures). These tests invoke `tcpdump` or `tshark` binaries which are not installed in this environment. Running `which tcpdump` and `which tshark` both return "not found." The remaining 2 failures in `regression.uts` are `conf.use_pcap` configuration (1) and `manuf DB methods` (1).
2. **Tests requiring hardware interfaces** — 43 failures across `isotp_native_socket.uts` (28 failures) and `isotpscan.uts` (15 failures). These tests require a CAN bus interface (`vcan0`) which is not available in this environment.
3. **Other environment-specific failures** — 4 failures: missing `brotli` library for HTTP decompression (`http.uts`), DHCP alias definition (`dhcp.uts`), ARPing output format (`l2.uts`), and a UDS scanner dissection edge case (`uds_scanner.uts`).

These are **not core Scapy logic failures** — they are environmental constraints of running the test suite in a containerized non-root environment without the optional external tools (`tcpdump`, `tshark`) and hardware interfaces (CAN bus) that some tests require.
