...
> **What this document answers.** How Scapy builds Ethernet frames, specifically: (a) whether it auto‑pads short payloads up to the minimum Ethernet frame size, (b) whether that padding persists across a `raw()` serialize / re‑parse round‑trip, and (c) how it dispatches to the next protocol from the `EtherType` field — including the *unrecognized* type case. It is an **evidence‑backed, read‑only investigation**: every behavioral claim below sits next to the exact observed output line that proves it, and every code claim carries a `file:line` reference. **No existing repository source/test/config/build file was modified; the only repository change is this new documentation file.**
...
|------|-------|---------------|
| Scapy source baseline (under investigation) | `0925ada485406684174d6f068dbd85c4154657b3` | an **ancestor** of the delivery HEAD — `git merge-base --is-ancestor 0925ada485406684174d6f068dbd85c4154657b3 HEAD; echo $?` → `0`; the `scapy/` tree is **byte‑identical** at HEAD (see provenance note below) |
| Delivery branch | `blitzy-490db774-82fc-4779-a5f2-60e475d50b67` | `git branch --show-current` |
| Delivery HEAD | a descendant of the baseline that adds only this document; `1138e7b6b4ba170d0e947cdcc1f641544aee6052` **as observed during final QA** | `git rev-parse HEAD` (this value advances as this document is revised; it is **not** the source baseline) |
...
- **Source baseline vs. delivery HEAD (provenance).** Every `file:line` citation and behavioral measurement in this document was investigated against Scapy **source commit `0925ada485406684174d6f068dbd85c4154657b3`**. That commit is an **ancestor** of the current delivery HEAD, and the only commits layered on top of it add *this* documentation file — so the `scapy/` source tree is **byte‑identical** at HEAD. Evidence: `git diff --stat 0925ada485406684174d6f068dbd85c4154657b3 HEAD -- scapy/` prints nothing (empty), and `git merge-base --is-ancestor 0925ada485406684174d6f068dbd85c4154657b3 HEAD; echo $?` prints `0`. Consequently `git rev-parse HEAD` returns the **delivery** commit — **not** the source baseline: it was `1138e7b6b4ba170d0e947cdcc1f641544aee6052` when observed during final QA, on branch `blitzy-490db774-82fc-4779-a5f2-60e475d50b67` (`git branch --show-current`), and it advances further each time this document is revised. Because the `scapy/` source is identical to the baseline, every citation below resolves unchanged regardless of which delivery commit is checked out.
...
> The `src` MAC (`1a:47:be:8d:66:bd`) is **auto‑selected from the default interface** (`conf.iface`, here `eth0`) and is **not a meaningful protocol value** for this investigation. On this observation host it was **stable across repeated runs** because it reflects `eth0`'s hardware address — confirmed by `get_if_hwaddr(conf.iface) = 1a:47:be:8d:66:bd` and identical output across 5 separate processes; on a different host/interface it would instead take that environment's MAC. Do not treat it as a fixed or meaningful value. Everything else reproduces exactly.
...
A third, supplementary script captures **explicit no‑error evidence** for Q3 by wrapping the unknown‑EtherType parse (both `0x9000` and `0x1234`) in a `try/except` that flips a flag, paired with the process exit status:

```python
# /tmp/obs_work/obs_q3_noerror.py  (OUTSIDE the repo; read-only; delete after use)
from scapy.all import Ether, Raw, raw
ok = True
try:
    for et in (0x9000, 0x1234):
        reparsed = Ether(raw(Ether(type=et)/Raw(load=b"B"*10)))
        _ = reparsed.payload.__class__.__name__   # exercise dissection
        _ = repr(reparsed)                          # exercise the print()/show() path
except Exception:
    ok = False
print("Q3 no exception = %s" % ok)
print("Q3 unknown EtherType parse succeeded = %s" % ok)
```

Run command with **verbatim captured stdout and exit status**:

```
$ PYTHONPATH=<repo> python3 /tmp/obs_work/obs_q3_noerror.py; echo "exit=$?"
Q3 no exception = True
Q3 unknown EtherType parse succeeded = True
exit=0
```
...
**Explanation.** `64 = 14 (Ether) + 20 (IP) + 20 (TCP) + 10 (payload)` — **no padding is added.** The frame is 64 bytes purely because the 10 payload bytes are appended to the 54‑byte B1 total; Scapy inserts nothing to reach any minimum size. (Do not read a wire minimum into the number 64 here. The classic Ethernet minimum frame is **60 bytes without the FCS** — a 14‑byte header plus a 46‑byte minimum data field — or **64 bytes once the 4‑byte FCS is included**; but Scapy's `raw()` output excludes the FCS entirely and adds no minimum‑frame padding. This stack's `raw()` is 64 bytes *solely* because `14 + 20 + 20 + 10 = 64`, not because Scapy floored it to a 60‑ or 64‑byte wire minimum. The size‑sweep in Q4 confirms the total is always exactly `header + payload`, never floored.)
...
- **Observed (verbatim):** `B3alt type int repr = 4660`
- **Observed (verbatim):** `B3alt payload class after Ether: Raw`
...
- **(a) A raw data dump — ✅ YES, this is what happens.** After the `Ether` layer you get a `###[ Raw ]###` layer.
  - **Evidence (verbatim):** `payload class after Ether: Raw`, and the `show()` output ends with:
    ...
    ###[ Raw ]### 
         load      = 'BBBBBBBBBB'
    ```
  - **Literal `print()` evidence (verbatim).** The question names `print()` specifically. `print(pkt)` emits Scapy's compact one‑line summary; for **both** unknown types it ends in `/ Raw` and shows the type as hex — a raw dump, **no** error, **no** protocol guess. Command and captured output:

    ```python
    for et in (0x9000, 0x1234):
        print(Ether(raw(Ether(type=et)/Raw(load=b"B"*10))))
    ```

    ```
    1a:47:be:8d:66:bd > ff:ff:ff:ff:ff:ff (0x9000) / Raw
    1a:47:be:8d:66:bd > ff:ff:ff:ff:ff:ff (0x1234) / Raw
    ```

    `print()`'s summary, `repr()`'s `<Ether … |<Raw … >>` form, and `show()`'s detailed tree are three different renderings — **all three agree the layer after `Ether` is `Raw`**. (The leading `src` MAC is the auto‑selected interface address noted earlier and is not a meaningful value.)

- **(b) An error — ❌ NO.** No exception is raised and dissection of the unknown‑EtherType frame completes cleanly.
  - **Evidence (verbatim):** `Q3 no exception = True`
  - **Evidence (verbatim):** `Q3 unknown EtherType parse succeeded = True`
  - **Evidence (command exit status, verbatim):** `exit=0`
  - The parse of **both** `0x9000` and `0x1234` was wrapped in a `try/except` that flips a flag (the third supplementary script above); the flag printed `True` and the process exited `0`, confirming no traceback. (Consistently, had an error occurred, `payload class after Ether: Raw` in (a) would never have printed either.)
...
> Related to Q2: the *dissection‑side* counterpart is `Packet.extract_padding` [`scapy/packet.py:982-990`], whose base implementation returns `(s, None)` at [`scapy/packet.py:990`]. Layers that declare a length override it to peel surplus trailing bytes off into padding — e.g. `IP.extract_padding` [`scapy/layers/inet.py:553-557`] computes `tmp_len = self.len - (self.ihl << 2)` [`scapy/layers/inet.py:554`] and returns `s[:tmp_len], s[tmp_len:]` [`scapy/layers/inet.py:557`], using `IP.len` to split the declared payload from any surplus trailing bytes. That is the mechanism by which genuine surplus bytes (Q2b) reappear as a `Padding` layer while the ordinary short packet (Q2a) does not.
...
- **Any minimum‑length padding is per‑protocol opt‑in, not a global Ethernet behavior** — the contrib layer `scapy/contrib/pnio_dcp.py` defines its own `MIN_PACKET_LENGTH = 44` [`scapy/contrib/pnio_dcp.py:32`], preceded by the comment `# minimum packet is 60 bytes.. 14 bytes are Ether()` [`scapy/contrib/pnio_dcp.py:31`], and applies it inside that layer's own `post_build` via `padding = MIN_PACKET_LENGTH - (len(pkt + pay))` [`scapy/contrib/pnio_dcp.py:664`] — illustrating that minimum‑length handling is defined by a specific protocol layer, not by the core Ethernet build path.
...
| **I1** where padding is decided | Part 3 · I1 | `scapy/packet.py:742/744/746/758/767`, `scapy/packet.py:1906/1909-1911/1913-1918`; no min‑frame const in `scapy/data.py` |
| **I2** EtherType → next protocol | Part 3 · I2 | `guess_payload_class` `scapy/packet.py:1062-1079`; `bind_layers` `scapy/layers/inet.py:1101`, `scapy/layers/inet6.py:4083`, `scapy/layers/l2.py:687-695`; `dispatch_hook` `scapy/layers/l2.py:267-272` |
| **I3** unrecognized‑number logic | Part 3 · I3 | `default_payload_class` `scapy/packet.py:1081-1090` → `conf.raw_layer = Raw` `scapy/packet.py:1921` |
