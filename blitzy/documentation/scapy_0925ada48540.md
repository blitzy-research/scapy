...
`Source: scapy/config.py:364-372`
...
The `ScapyFreqFilter` (lines 42-75) **throttles repeated warnings**: after 2 occurrences from the same call site within `conf.warning_threshold` seconds, subsequent warnings are suppressed. This prevents flooding the console when many unroutable packets are sent in a loop.
...
3. The first 2 occurrences pass through (the second with "more " prepended to the message).
4. Subsequent occurrences within the threshold window are suppressed.
...
flowchart TD
    ...
    B2["conf.raise_no_dst_mac = True"] -.-> N["If getmacbyip returns None"]
    N --> O["DestMACField.i2h()"]
...
# Subsequent warnings suppressed within conf.warning_threshold window
```

`Source: scapy/error.py:42-75` — The filter allows 2 warnings (the second prefixed with "more "), then suppresses further occurrences from the same call site until the threshold window expires.
