...
|---------------|--------|--------|----------|
| `test/regression.uts` | 212 | 67 | Core regression tests — likely tests requiring network/root access |
...
| `test/scapy/layers/inet6.uts` | 490 | 9 | IPv6 tests — filtered by `kw_ko` but some still present |
| `test/answering_machines.uts` | 0 | 7 | Answering machines — require socket access |
| `test/pipetool.uts` | 24 | 5 | Pipe tools — timing/resource dependent |
| `test/scapy/layers/l2.uts` | 8 | 3 | Layer 2 — requires raw socket access |
| `test/cert.uts` | 61 | 2 | Certificate tests — TLS class dependencies |
| `test/linux.uts` | 1 | 2 | Linux-specific — requires root for some tests |
| `test/scapy/layers/inet.uts` | 52 | 2 | IPv4 layer — some tests need network access |
| `test/contrib/gxrp.uts` | 7 | 2 | GXRP contrib — environment-specific |
| `test/tls.uts` | 80 | 1 | TLS tests — class loading dependencies |
| `test/p0fv2.uts` | 11 | 1 | p0f v2 — fingerprint database dependent |
| `test/scapy/layers/dhcp.uts` | 6 | 1 | DHCP — requires network interface |
| `test/scapy/layers/http.uts` | 8 | 1 | HTTP — environment-specific |
| `test/contrib/dtp.uts` | 2 | 1 | DTP contrib — environment-specific |
| `test/contrib/automotive/scanner/uds_scanner.uts` | 28 | 1 | UDS scanner — dissection edge case |

**Failure context:** The majority of failures fall into three categories:
1. **Tests requiring raw sockets or root privileges** — Running with `-N` (non-root) skips most root-required tests, but some edge cases in `regression.uts`, `answering_machines.uts`, `l2.uts`, and `linux.uts` still fail because they attempt socket operations.
2. **Tests requiring hardware interfaces** — `isotp_native_socket.uts` and `isotpscan.uts` require a CAN bus interface which is not available in this environment.
3. **Environment-specific failures** — Some tests depend on specific network configurations, DNS resolution, or timing behavior that varies across environments.

These are **not core Scapy logic failures** — they are environmental constraints of running the test suite in a containerized non-root environment.
