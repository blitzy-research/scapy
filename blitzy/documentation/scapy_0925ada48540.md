...
The `Ether.payload_guess` list contains **16 registered bindings** as of this commit, loaded by default via `from scapy.all import *` — 14 from `bind_layers()` calls in `scapy/layers/` modules, 1 from `bind_bottom_up(Ether, EAPOL, type=0x888e)` in `scapy/layers/eap.py:852`, and 1 from `bind_layers(Ether, EtherCat, type=0x88a4)` in `scapy/contrib/ethercat.py:620` (auto-loaded because `scapy/layers/bluetooth4LE.py` imports from `scapy.contrib.ethercat`):
...
| `0x88A4` | 34980 | `EtherCat` | `scapy/contrib/ethercat.py:620` |
...
flowchart TD
    ...
    C --> D["post_dissect(s)"]
    D --> E["extract_padding(s) → payl, pad"]
    E --> F["do_dissect_payload(payl)"]
    F --> G{"pad is non-None<br/>AND conf.padding?"}
    G -->|"Yes"| H["add_payload(Padding(pad))"]
    G -->|"No"| I["Done"]
    H --> I
```
