...
*(Continuation of the same `$ python3 /tmp/tcp_overlap_test.py` execution shown above)*
...
sequenceDiagram
    ...
    participant Async as AsyncSniffer._run()<br/>sendrecv.py:1064
...
3. `guess_payload_class()` at `scapy/layers/http.py:637` attempts to parse `HTTP/1.0 2ZZZZZZZZZZtent-Lengt`:
   - Inside the `try` block, `payload.index(b"\r\n")` at `scapy/layers/http.py:647` raises `ValueError` because the corrupted 30-byte buffer contains **no `\r\n` characters** — the Z-bytes at positions `[10:20)` overwrote the original CRLF that was at positions 15–16.
   - The `except ValueError:` block at `scapy/layers/http.py:657-659` catches the exception.
   - Execution falls through to `return Raw` at `scapy/layers/http.py:660`.
   - **Note**: The regex matching code at lines 642–656 (request regex at lines 643–645, response regex at line 653) is **never reached** for this corrupted input because the `ValueError` occurs before any regex is applied.
