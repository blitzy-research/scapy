...
| Current `HEAD` (this doc commit) | `0a0a3437` (`git rev-parse --short HEAD`) — the commit that adds this document; distinct from the Scapy source baseline above, which is unchanged |
...
- **Interactive-console captures** (Q1 banner, Q5 interactive theme) are shown from the
  **combined stream** (`2>&1`) with **ANSI color codes stripped for readability** (the real
  output *is* colored — see Q5); the strip is done with
  `sed 's/\x1b\[[0-9;]*m//g'`. These blocks include the leading startup log lines *and* the
  trailing `>>> now exiting InteractiveConsole...` line that the shell prints on exit.
- **Programmatic `python3 -c` captures** are shown as **stdout only** by appending `2>/dev/null`
  to the command. On this system every `from scapy.all …` import writes **two identical
  `TripleDES … CryptographyDeprecationWarning` lines to stderr** (from `cryptography` v43.0.0 —
  see Q1 for the exact text and `[scapy/layers/ipsec.py:573]`/`[scapy/layers/ipsec.py:577]`);
  `2>/dev/null` suppresses exactly those two lines so the block shows only the values of
  interest. Any command that is expected to write to stderr (e.g. `ls` of a missing file) is
...
4. `_version_from_git_describe()` `[scapy/__init__.py:151-155]` — fails because
   ...
   history carries no version tag), so the tag parse raises and is caught. *(Verified now, and
   note the two commits are distinct: the unqualified command reports the current `HEAD`,
   `git describe --tags --always --long` → `0a0a3437` (the commit that adds this document); the
   Scapy source baseline is likewise untagged, `git describe --tags --always --long 0925ada4` →
   `0925ada4`. Either way the result is a bare commit, so this step is skipped. When Scapy
   authored the version, `_version()` ran against the same untagged history, hence the fallback
   below.)*
...
Verified two ways (stdout only, `2>/dev/null`):
...
python3 -c "import os,datetime; print(datetime.datetime.fromtimestamp(os.path.getmtime('scapy/__init__.py'), datetime.UTC).strftime('%Y.%m.%d'))"
```
...
`[scapy/sendrecv.py:355]` (`sndrcv`), and `[scapy/sendrecv.py:730]` (`__sr_loop`). Each operation
...
**Exact command — active theme in the interactive session** (stdout only, ANSI stripped, like
Q1):
...
- **Read-only:** no Scapy source file was modified, created, or deleted. Every `scapy/*` path
  ...
  `A  blitzy/documentation/scapy_0925ada48540.md`, and all 13 cited source files are
  byte-identical to that baseline.
- **Commits:** the Scapy source baseline is commit `0925ada4`, while the current `HEAD`
  (`git rev-parse --short HEAD` → `0a0a3437`) is the commit that adds this document. The two are
  distinct — the Scapy source tree itself is unchanged from the baseline.
