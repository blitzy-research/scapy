...
**Provenance note — applies to every producing snippet in this document.** In the commands below, `./.venv/bin/python` is the repository virtualenv interpreter (CPython `3.13.7`) created per the project setup instructions and run from the repository root; `PYTHONPATH=.` imports Scapy in place and `SCAPY_USE_LIBPCAP=no` disables the optional libpcap backend (no root privileges, no live network). The trailing `2>/dev/null` discards a single import-time `CryptographyDeprecationWarning` that Scapy prints on **stderr** (`TripleDES has been moved ...`, from `scapy/layers/ipsec.py:573`); it is a warning only and does not affect any captured stdout value quoted below. All six behaviors are reproducible purely by crafting packets in memory and calling `.hashret()` / `.answers()` directly.
