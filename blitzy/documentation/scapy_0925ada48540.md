...
The relevant gating excerpt `[scapy/layers/inet.py:L595-L610]`:
...
- **The fixed defaults.** The ICMP `id` field defaults to `0` `[scapy/layers/inet.py:L957]` and `seq` defaults to `0` `[scapy/layers/inet.py:L958]`. Crucially, **`id` is never auto-randomized**: Scapy leaves ICMP `id` at the literal default `0` unless you set it. (Scapy's IP `id` field is *likewise* a fixed default — `ShortField("id", 1)` `[scapy/layers/inet.py:L528]` — and is **not** randomized by Scapy either; it varies in practice only because a caller explicitly sets `id=RandShort()` — as `traceroute` does `[scapy/layers/inet.py:L1849,L1854]` — or because the OS kernel assigns it when the packet is transmitted.) So if your tool leaves `id` alone, only `seq` can vary between probes (and only if you explicitly set `seq`).
...
The primary evidence above is the in-repository `path:line` citations and the empirically reproduced bytes. The following external sources corroborate (but do not replace) that code analysis; each entry includes a short quoted excerpt from the linked source:

- **Scapy usage documentation** — confirms Scapy normally expects replies from the IP the stimulus was sent to, and that `conf.checkIPaddr = False` is the documented switch for the DHCP broadcast case where that is not true. Quoting the page: "Scapy normally makes sure that replies come from the same IP address the stimulus was sent to." <https://scapy.readthedocs.io/en/latest/usage.html>
- **Scapy configuration API documentation** — confirms the default flag values used here (`checkIPaddr`, `checkIPsrc`, `checkIPinIP` true; `checkIPID` false). On `checkIPinIP` it states: "if True, checks that IP-in-IP layers match". <https://scapy.readthedocs.io/en/stable/api/scapy.config.html>
- **Scapy source — `scapy/layers/inet.py` (upstream `master`)** — the authoritative origin of the `IP.hashret`/`IP.answers` bodies quoted above; addresses are folded into the bucket key only under the guard "if conf.checkIPsrc and conf.checkIPaddr:". <https://github.com/secdev/scapy/blob/master/scapy/layers/inet.py>
- **GitHub issue secdev/scapy#383** — context on encapsulated/IP-in-IP responses; the report is titled "Answers and IP in IP". <https://github.com/secdev/scapy/issues/383>
- **GitHub PR secdev/scapy#387** — introduced `conf.checkIPinIP`; per its description: "This setting, when set to False, will exclude IP layers which encapsulate another IP layer". <https://github.com/secdev/scapy/pull/387>
