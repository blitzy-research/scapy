...
$ git describe --tags --always --long                                            -> 27651b28
```

> **Command context (reproducibility note).** `_version_from_git_describe()` runs the **bare** command `git describe --tags --always --long` [`scapy/__init__.py:L114`] at *import time*, i.e. against whatever commit is currently checked out. On the **source checkout under study** (`0925ada485406684174d6f068dbd85c4154657b3`) that bare command yields **`0925ada4`**; to reproduce that exact value on any later checkout, pass the source commit explicitly (first line above). On the **delivered branch** — where a documentation commit sits on top of the source — the same bare command instead reports the *documentation* commit **`27651b28`** (third line above). Either way the string is a bare short hash, not a `vX.Y.Z` tag, so the version derivation below is unaffected.
...
- `git describe --tags --always --long` returns only a **bare short hash** — **`0925ada4`** on the source checkout `0925ada485406684174d6f068dbd85c4154657b3` (and `27651b28` on the delivered documentation commit) — because the repository has **0 tags**; that string does not match the expected `vX.Y.Z` tag shape, so `_version_from_git_describe()` raises `ValueError: tag has invalid format` → Method 3 skipped. (Whichever commit is checked out, the outcome is the same: a non‑tag hash that fails the format check.)
...
Every run used the flags `-t <file> -qq -N` (`-qq` = silent, printing only the CRC/SHA and summary; `-N` = force non‑root, for reproducibility). The runner prints a version banner and a per‑campaign `PASSED=/FAILED=` line:
