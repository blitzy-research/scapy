...
| `scapy/base_classes.py` | `Packet_metaclass` (L281, `__all_slots__` at L352): builds `__all_slots__` set used by `__setattr__` |
...
def _raw_packet_cache_field_value(self, fld, val, copy=False):
    ...
    if fld.holds_packets:                          # L652
        if fld.islist:                             # L654
            return [
                _cpy(x.fields) for x in val        # L655-657: ONLY tracks x.fields!
            ]
        else:
...
4. The sub-packet's cache is cleared at L484-487:
   ```python
   self.explicit = 0
...
# PROBLEM: bytes(pkt) returns a MALFORMED packet — the Raw layer rebuilds with the new
# content (setfieldval() clears Raw's cache at L485), but IP/UDP headers retain stale
# len and chksum values because their caches are still valid and post_build() is skipped
# by the cache gate at L737. The result is stale 20+8 byte headers + 31-byte new payload.
