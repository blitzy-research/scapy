...
```python
# scapy/layers/inet.py L1000–L1003
...
flowchart TD
    ...
    subgraph Delegation["answers() delegation for an ICMP error"]
        D1["IP.answers(): payload.type in 3,4,5,11,12  [L600]"] --> D2["recurse into embedded IPerror  [L604]"]
...
The send phase and receive phase share a key space through `hashret()`; on the receive side, the `answers()` confirmation for an ICMP error follows the delegation chain `IP.answers → (ICMP-error branch) → IPerror.answers → L4error.answers`.
...
**Scapy extracts the embedded original packet and compares its fields — it does *not* compare the outer ICMP error header.** When `IP.answers()` recognizes that `self` is an ICMP error, it discards the outer (router→host) header entirely and recurses into the **embedded citation**, comparing that against the original probe.

### 4.2 Source

`IP.answers()` detects the error condition — proto is ICMP, the payload is `ICMP`, and the type is one of `[3, 4, 5, 11, 12]` — and recurses into the embedded packet:

```python
# scapy/layers/inet.py L600–L610 — IP.answers() (ICMP-error branch)
...
```text
IP.answers()  →  (ICMP-error branch, L600–L604)  →  IPerror.answers()  →  ICMPerror/TCPerror/UDPerror.answers()
```
