...
| Commit (`git rev-parse --short HEAD`) | `0925ada4` |
...
**Independent checksum cross-check.** Beyond re-dissection, the R1 wire bytes were fed to a
hand-rolled RFC-793 one's-complement implementation: the IP checksum recomputes to `0x66c8` and the
full 20-byte header folds to `0x0000`; the TCP checksum recomputes to `0x37a5` and the
pseudo-header + segment folds to `0x0000`. Both emitted checksums are therefore mathematically valid
over the exact emitted bytes.
...
- Mutating any field via **`Packet.setfieldval`** (`scapy/packet.py:472-491`) resets
  `explicit = 0` (`:484`), `raw_packet_cache = None` (`:485`),
  `raw_packet_cache_fields = None` (`:486`), and `wirelen = None` (`:487`). Deleting a field via
  **`Packet.delfieldval`** (`scapy/packet.py:504-515`) performs the same resets (`:508-511`).
...
graph TD
    ...
    F["set/del any field<br/>setfieldval (472-491) / delfieldval (504-515)"] --> G["raw_packet_cache = None<br/>explicit = 0"]
    G --> C
...
- The assignment path is **`Packet.setfieldval`** at `scapy/packet.py:472-491`, which stores the
...
- `d.ttl = 42` goes through **`Packet.setfieldval`** (`scapy/packet.py:472-491`), which clears
...
- `del d.chksum` goes through **`Packet.delfieldval`** (`scapy/packet.py:504-515`), which removes the
