...
> **(inferred)**.
>
> **Commit pin.** All citations are pinned to the **source baseline commit**
> `0925ada485406684174d6f068dbd85c4154657b3` (branch `scapy_0925ada48540`) — the immutable
> commit every line number below was validated against. This baseline is an *ancestor* of the
> current deliverable `HEAD`; committing this document necessarily advances `HEAD` beyond the
> baseline, so a bare `git rev-parse HEAD` in the delivered checkout returns the deliverable
> commit, not this baseline. §2 gives reproducible proof that `scapy/layers/dns.py` is
> byte‑identical at the baseline and at the current `HEAD`, so every citation below stays
> exact. At this baseline commit, `scapy/layers/dns.py` is **1179 lines**; every line number below
> was re‑read against that exact file.
...
**Source baseline vs. deliverable commit.** The commit pin (`0925ada4…`) is the *immutable
source baseline* — the commit every `file:line` citation was validated against. It is
deliberately **not** shown as the output of a bare `git rev-parse HEAD`, because committing this
document advances `HEAD` beyond the baseline: in the delivered checkout `git rev-parse HEAD`
returns the *deliverable* commit, not the baseline. That is expected and harmless, because the
baseline is an **ancestor** of `HEAD` and `scapy/layers/dns.py` is **byte‑identical** at
both commits. The checks below are stable and reproducible from the delivered checkout —
they depend only on the fixed baseline hash and on the content of `scapy/layers/dns.py`, never on
the mutable `HEAD` hash:

```text
$ git merge-base --is-ancestor 0925ada485406684174d6f068dbd85c4154657b3 HEAD && echo ok
ok
$ git rev-parse 0925ada485406684174d6f068dbd85c4154657b3:scapy/layers/dns.py
9dc1a19d9ff570f734cefa862828489cdc53a828
$ git rev-parse HEAD:scapy/layers/dns.py
9dc1a19d9ff570f734cefa862828489cdc53a828
```

The first check confirms the baseline is an ancestor of the current `HEAD`; the two identical blob
hashes prove `scapy/layers/dns.py` was never modified between the baseline and the delivered
`HEAD`, so every `file:line` anchor in this document resolves exactly.
...
| `StrField` (base of the DNS string fields) | Supporting surface | [scapy/fields.py:L1414] |
| `StrLenField` (base of `DNSStrField`/`DNSTextField`) | Supporting surface | [scapy/fields.py:L1888] |
| `DNS_am` (server‑side build path) | Supporting surface | [scapy/layers/dns.py:L1114] |
...
**Supporting code surface.** The DNS name fields are built on Scapy's generic string fields.
Observed class chain (runtime MRO): `DNSStrField` [scapy/layers/dns.py:L279] → `StrLenField`
[scapy/fields.py:L1888] → `StrField` [scapy/fields.py:L1414]; `DNSRRField`
[scapy/layers/dns.py:L336] → `StrField`; `DNSQRField` [scapy/layers/dns.py:L399] →
`DNSRRField`; and `DNSTextField` [scapy/layers/dns.py:L408] → `StrLenField`. These base
classes supply the raw length/byte get/set that each DNS‑specific `getfield` wraps around
`dns_get_str`. Separately, the server‑side answering machine `DNS_am`
[scapy/layers/dns.py:L1114] assembles a reply in `make_reply` [scapy/layers/dns.py:L1150-L1179] with the verbatim
statement `resp /= self.cls(id=dns.id, qr=1, qd=dns.qd, an=DNSRR(rrname=dns.qd.qname, ttl=10, rdata=rdata))` (where `self.cls` is `DNS`). Observed at
runtime it uses the **default non‑compressing** `bytes(pkt)` build path (consistent with §3.1):
for `DNS(id=1, qr=1, qd=DNSQR(qname='www.example.com'), an=DNSRR(rrname='www.example.com', ttl=10,
rdata='1.2.3.4'))`, `len(bytes(pkt))` is **64** with **no** `0xc0` pointer, and only an explicit
`.compress()` reduces it to **49** bytes carrying a `c00c` pointer. `DNS_am` therefore travels the
shared name‑*encoding* path (`dns_encode`) but does **not** invoke the explicit `dns_compress`
opt‑in; its broader server logic is beyond this document's name‑compression scope.
...
to the repository. The only file added to the repository is this document.

Because this document is itself the file being added, the repository's *working‑tree* state
necessarily changes when it is committed: before the first commit the document is an **untracked**
file; after it is committed the working tree is **clean**. The authoritative, reproducible proof of
the **final delivered state** — stable no matter how many times the document is revised and
re‑committed — is therefore expressed with commands that do **not** depend on the
mutable working‑tree or `HEAD` hash:

```text
$ git status --porcelain
$            # (empty — clean working tree; the deliverable is committed)
...
$            # (empty — no Scapy source, test, doc, or config file changed)
```

The first command shows a **clean** working tree; the second shows the **only** file differing from
the source baseline is this document; the third — which excludes the document — is
empty, proving **no** existing Scapy source, test, documentation, or configuration file was
modified. Together with the byte‑identical `scapy/layers/dns.py` blob hash shown in §2,
this is complete read‑only proof.

For completeness, the block below is the **historical, point‑in‑time capture** taken
from the repository root *immediately before the document was first committed* (while it was still
untracked). It is a snapshot of that earlier moment and is **not** expected to reproduce in the
delivered checkout, where the file is already committed and the tree is clean:

```text
$ git status --porcelain              # (historical: before the first commit)
?? blitzy/
...
The single untracked entry resolved to exactly this one new file (in its new `blitzy/documentation/`
directory). This document was then staged and committed as the sole change on the working branch.
