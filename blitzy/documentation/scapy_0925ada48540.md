...
- **Python:** `3.12.3` (observed; satisfies `requires-python = ">=3.7, <4"` — `pyproject.toml:L17`).
...
**Raw output (RUN #2 — stability):** the frame counts are identical to RUN #1 (`6->1`, `7->1`, `8->2`, `20->3`):
...
**(a) Timeout value.** The applicable timer is the ISO 15765-2 **N_Cr** consecutive-frame timer, modeled in Scapy as **`cf_timeout = 1` second**. The measured elapsed time from the first Consecutive Frame (CF1) to the receive-state reset was **1.002 s**, stable across 3 runs. Cite `scapy/contrib/isotp/isotp_soft_socket.py:L496-L497` (`self.fc_timeout = 1` / `self.cf_timeout = 1`).
...
[DEFAULT verb=2 RUN1] conf.verb=2
  ...
  AFTER:  rx_state=ISOTP_IDLE(0)   elapsed CF1->reset = 1.002s
...
[DEFAULT verb=2 RUN2] conf.verb=2
  ... (identical) ... elapsed CF1->reset = 1.002s ... records = (none)
...
SUMMARY cf_timeout=1s; measured CF1->reset run1=1.002s run2=1.002s run3=1.002s
```

The measured `cf_timeout` of **~1.002 s** is **stable across 3 runs**.
...
No repository file was modified. The temporary observation scripts `/tmp/isotp_probe{1..4}.py` were created **outside** the repository tree, executed to capture the evidence above, and then deleted; generated `__pycache__` was removed. `git status --porcelain` (tracked) and `--ignored` both return **empty** — the working tree is byte-for-byte unchanged apart from the new `blitzy/documentation/scapy_0925ada48540.md` (this document).
...
- `pyproject.toml:L17` — `requires-python = ">=3.7, <4"` (Python 3.12.3 satisfies it).
