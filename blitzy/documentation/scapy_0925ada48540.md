...
- Importing Scapy prints a cosmetic `CryptographyDeprecationWarning` from `scapy/layers/ipsec.py`; it does not
  affect build output and is silenced in the script (`warnings.filterwarnings("ignore")`) for clean capture.
...
"""
import warnings
warnings.filterwarnings("ignore")  # silence cosmetic CryptographyDeprecationWarning
