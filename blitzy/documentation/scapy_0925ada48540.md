...
- `conf.debug_dissector` **observed** as `False`. *(Inferred from source:* declared `debug_dissector = False` at `scapy/config.py:817`; this binary flag gates the dissection path — deciding whether a failed layer construction is re-raised or silently wrapped in `Raw`. It is checked at two points on that path (a failed child-layer guess at `scapy/packet.py:1037` and a failed `dispatch_hook` at `scapy/base_classes.py:389`), each a plain truthiness test — see Sections 6 and 9.*)*
...
**Observed behavior — the dissection gate is binary.** The layer-handoff path branches on `conf.debug_dissector` at **two places**, and **each is a plain truthiness test — never a graded `>= 1` / `>= 2` comparison**. The **first** gate governs a **failed child-layer construction** on the `guess_payload_class` path, at `scapy/packet.py:1037`:
...
The **second** gate governs a **failed `dispatch_hook`**, one level earlier — in the packet **metaclass** `Packet_metaclass.__call__` (`scapy/base_classes.py:379`). Before an instance is constructed, if the class defines a `dispatch_hook` it is invoked at `scapy/base_classes.py:386`; when that hook *itself* raises, the same binary flag decides the outcome at `scapy/base_classes.py:389` (re-raise at `:390`, else fall back to `conf.raw_layer` at `:391`):
...
**Live, observed demonstration of the second gate.** `GRE(b'\x00')` reaches this branch through the canonical entry point (instantiating a layer from raw bytes): `GRE.dispatch_hook` (`scapy/layers/l2.py:598-603`) runs `struct.unpack("!H", _pkt[2:4])`, and a 1-byte buffer makes `_pkt[2:4]` empty, so the hook raises `struct.error` *inside* the metaclass — exercising the `scapy/base_classes.py:389` gate rather than the `scapy/packet.py:1037` one. This is a **different** gate from the `guess_payload_class` child-construction path demonstrated in OBJ-3 (Section 6): it fires *before* the guessed class is ever constructed.

```bash
$ mkdir -p /tmp/scapy_qa
$ cat > /tmp/scapy_qa/gate2.py <<'PY'
import sys
from scapy.all import *

conf.color_theme = NoTheme()  # deterministic, uncolored output for the document

# GRE(b'\x00') is the canonical entry point: instantiate a layer from raw bytes.
# GRE.dispatch_hook (scapy/layers/l2.py:598-603) runs struct.unpack("!H", _pkt[2:4]);
# a 1-byte buffer makes _pkt[2:4] empty, so the hook raises struct.error *inside*
# Packet_metaclass.__call__ at scapy/base_classes.py:386 -> caught at :387 -> the
# SECOND debug gate at scapy/base_classes.py:389 (distinct from packet.py:1037).

print("=== default conf.debug_dissector =", conf.debug_dissector, "===", flush=True)

print("=== debug_dissector = False: GRE(b'\\x00') ===", flush=True)
conf.debug_dissector = False
p = GRE(b"\x00")
print("class =", p.__class__.__name__, "| layers =",
      [c.__name__ for c in p.layers()], flush=True)

print("=== debug_dissector = True: GRE(b'\\x00') ===", flush=True)
conf.debug_dissector = True
try:
    GRE(b"\x00")
    ...
    conf.debug_dissector = False
    print("restored conf.debug_dissector =", conf.debug_dissector, flush=True)
PY
$ PYTHONPATH="$(git rev-parse --show-toplevel)" python3 /tmp/scapy_qa/gate2.py 2>&1
```
...
/tmp/blitzy/scapy/blitzy-d51044c1-9295-46b6-96ce-fde0e766cd98_8ab594/scapy/layers/ipsec.py:577: CryptographyDeprecationWarning: TripleDES has been moved to cryptography.hazmat.decrepit.ciphers.algorithms.TripleDES and will be removed from this module in 48.0.0.
  cipher=algorithms.TripleDES,
=== default conf.debug_dissector = False ===
=== debug_dissector = False: GRE(b'\x00') ===
class = Raw | layers = ['Raw']
=== debug_dissector = True: GRE(b'\x00') ===
RAISED: error - unpack requires a buffer of 2 bytes
restored conf.debug_dissector = False
```

Two-run stability (R2): `sha256(stdout+stderr) = 4aa66a4b466f8cd7aebadacd7e7b4e808d263397b560de97215897f9cb9522fc`, identical across two consecutive runs. **Observed:** under the default `conf.debug_dissector = False` the failed hook is swallowed and the leftover byte is wrapped in `Raw` (`class = Raw`, `layers = ['Raw']`, via `scapy/base_classes.py:391`); under `conf.debug_dissector = True` the original `struct.error` is re-raised unchanged (`scapy/base_classes.py:390`). Both gates on the dissection path are therefore **binary** — any truthy value re-raises, any falsy value falls back to `conf.raw_layer` — with no intermediate level.
