...
> *documented*/CI runtime is Python 3.11 (`tox.ini` `envlist` up to `py311`); Python 3.12.3
> is within that declared range. Because the analyzed build/dissect/cache mechanism is
> pure-Python and version-agnostic across these releases, the reproductions produce
> **byte-for-byte identical output** on Python 3.11.x and Python 3.12.3 — observing the same
> hex on two different minor versions is itself corroboration of that version-independence.
...
(`doc/scapy/build_dissect.rst:L257`, `doc/scapy/build_dissect.rst:L262`).
...
They were executed on **Python 3.12.3** with **Scapy `VERSION 2026.06.26`** and removed when
the investigation concluded — they are never committed. (As noted in the abstract, the same
byte-for-byte output is produced on Python 3.11.x; the mechanism is pure-Python and
version-agnostic.) The output shown is the **verbatim** observed output.
...
executed against the source tree via `PYTHONPATH` on Python 3.12.3 / Scapy `2026.06.26`. No
