...
- **Environment-specific vs. general.** Several reported results are host/environment-specific and are labeled as such where they appear: **(a)** the version *digits* (`2026.07.08`) are derived from a file modification time; **(b)** the socket classes are selected for **Linux**; and **(c)** the loaded-**module** count `len(conf.load_layers) = 49` is **Linux/BSD-specific**, because it includes a `tuntap` module appended only under `if LINUX or BSD:` (`scapy/arch/__init__.py:L148`–`L149`) — on a non-Linux/BSD host it would be **48**. Most other results generalize across platforms — the verbosity default, the packet structure, the version-derivation and theme-*selection* mechanisms, and the code paths — but two more values are host-observed rather than universal: the registered-**class** count `len(conf.layers) = 1319` tracks *which modules are imported*, so it too is platform-sensitive (a non-Linux/BSD host that skips the `tuntap` module registers **3** fewer classes → **1316**); and the active console theme is `DefaultTheme` on this Linux host but `BlackAndWhite` on Windows without IPython (see Q5 and the closing note).
...
**Output** (every emitted line is shown in full — nothing is truncated; the only normalization is that the ANSI **color** codes were stripped with `sed` for readability, and the **complete raw colored capture with every escape code** is included immediately below. The quote on the last banner lines is randomly selected — this run drew the "Sebastien Chabal" quote):
...
**Raw ANSI colors — a single representative line** (same run as the banner above, the `Welcome to Scapy` logo/info line shown with its escape codes via `cat -v`, where `^[` = `ESC`):
...
**Raw ANSI colors — the complete banner** (every logo + info-column line of a fresh run shown with all escape codes via `cat -v`; the random quote naturally differs from the stripped capture above, but the green-logo / blue-info color structure is identical on **every** line):

```
$ echo "exit()" | PYTHONPATH=. python3 -m scapy 2>&1 | cat -v | sed -n '/aSPY\/\/YASa/,/ccaacs/p'
...
^[[32m^[[1m              P////YCpc           A//A^[[0m^[[34m^[[1m   | Craft packets like I craft my beer.^[[0m
^[[32m^[[1m       scccccp///pSP///p          p//Y^[[0m^[[34m^[[1m   |               -- Jean De Clerck^[[0m
...
So each **logo** line is wrapped in `ESC[32m` (green) + `ESC[1m` (bold) and each **info-column** line in `ESC[34m` (blue) + `ESC[1m` (bold), every span terminated by `ESC[0m` — confirmed on every line of the complete capture above, not just the single representative line.
...
- **(b) Non-fancy banner** — when `conf.fancy_prompt` is `False` the whole logo is skipped and `banner_text = "Welcome to Scapy (%s)" % conf.version` (`scapy/main.py:L657`). This is what the `-H` flag triggers: at `scapy/main.py:L531`–`L534` it sets `conf.fancy_prompt = False` (`L532`), `conf.verb = 1` (`L533`), and `conf.logLevel = logging.WARNING` (`L534`). The command below prints `conf.verb` and `conf.fancy_prompt` from *inside* the `-H` session, so the lowered verbosity is shown at runtime — not merely cited from code:

  ```
  $ printf '%s\n' 'print("VERB", conf.verb)' 'print("FANCY", conf.fancy_prompt)' 'exit()' | PYTHONPATH=. python3 -m scapy -H 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | grep -E 'Welcome to Scapy|VERB|FANCY'
  Welcome to Scapy (2026.07.08)
  >>> VERB 1
  >>> FANCY False
  ```

  The `Welcome to Scapy (2026.07.08)` line confirms the non-fancy banner, and `VERB 1` / `FANCY False` confirm that `-H` lowered `conf.verb` from its default `2` to `1` and disabled the fancy prompt.

- **(c) Random quote** — the full banner appends `quote, author = choice(QUOTES)` (`scapy/main.py:L646`) via `_prepare_quote(quote, author, max_len=39)` (`scapy/main.py:L647`), which is why the last banner lines **differ every startup**. Across four runs this environment drew four different quotes (e.g. the "Sebastien Chabal" quote shown above, an "IPv6 layer — Craft me if you can." quote, and a "Satoshi Nakamoto — Star us on GitHub!" quote). The quote is therefore **run-specific by design**.
...
- **`-H` modifier:** `scapy/main.py:L531`–`L534` sets `conf.fancy_prompt = False` (`L532`), `conf.verb = 1` (`L533`), and `conf.logLevel = logging.WARNING` (`L534`). This is **runtime-confirmed** in Q1 variant (b), where a `-H` session prints `VERB 1` and `FANCY False` — i.e. the default `conf.verb = 2` is lowered to `1`.
...
- **Generalizes across platforms and runs** (deterministic, code-driven):
  ...
  - **Q2 mechanism and static base** — the "files ≠ loaded" reasoning (registered *classes* ≠ source *files*, populated by the `Packet` metaclass) generalizes, and the **static base** of `conf.load_layers` is fixed across platforms at **48** entries (the literal at `scapy/config.py:L848`–`L897`); only the `tuntap` append is platform-conditional. The **exact** registered-class total, however, is *not* OS-independent (platform-conditional module loading changes it), so the specific value is listed under the environment-specific results below rather than here.
  ...
  - **Q5 theme-selection *mechanism*** — the interactive console installs a theme at startup (`conf.color_theme = ...` in `interact()`, `scapy/main.py:L514`) while a plain library import keeps the `Conf` default `NoTheme` (`scapy/config.py:L818`). The resulting console theme is `DefaultTheme` on this Linux host (and anywhere IPython is present); the one documented exception is **Windows without IPython**, where `interact()` installs `BlackAndWhite` instead (`scapy/main.py:L582`, inside `except ImportError:` at `L573` and `if WINDOWS:` at `L578`) — see the Q5 sibling-variant discussion. The exact console-theme *name* is therefore also listed under the environment-specific results below.
- **Host/environment-specific** (labeled as such above):
  ...
  - The **Q2 registered-class count** `len(conf.layers) = 1319`, which is the value **observed on this Linux host**. It is *not* strictly OS-independent: because the `tuntap` module loads only under `if LINUX or BSD:` (`scapy/arch/__init__.py:L148`–`L149`) and itself registers **3** `Packet` classes (`TunPacketInfo`, `LinuxTunIfReq`, `LinuxTunPacketInfo`), a non-Linux/BSD host that skips `tuntap` would register **1316**. More generally the total tracks *which modules are imported*, which can vary with platform-conditional or optional imports.
  ...
  - The **active console-theme name** — `DefaultTheme` here (and anywhere IPython is present), but `BlackAndWhite` on **Windows without IPython** (`scapy/main.py:L582`). The *mechanism* that installs it (`interact()` at `scapy/main.py:L514`) generalizes; only the resulting theme name is platform-conditional.
