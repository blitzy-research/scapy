...
3. If the key is `True`, the `USE_CAN_ISOTP_KERNEL_MODULE` flag is set to `True`.
4. If the key is missing, a `KeyError` is caught and an informational message is logged (lines 37–39).
5. Regardless of the key's value, `ISOTPNativeSocket` is imported and added to `__all__` on all Linux systems (lines 41–42).
6. The final assignment at lines 44–47 aliases `ISOTPSocket` to either `ISOTPNativeSocket` or `ISOTPSoftSocket`.
...
for size in [6, 7]:
    pkt = ISOTP(rx_id=0x641, rx_ext_address=0x55, data=bytes(range(1, size+1)))
    frames = pkt.fragment()
    print(f"Extended addr, {size} bytes: {len(frames)} frame(s)")
    for i, f in enumerate(frames):
        raw = bytes(f)
        data = raw[8:]
        print(f"  Ext{size} Frame {i}: data={data.hex()}")
```
...
cf1_data = bytes(frames[1])[8:]
print(f"CF1 data: {cf1_data.hex()}")
last_cf_data = bytes(frames[-1])[8:]
print(f"Last CF data: {last_cf_data.hex()}")
```
