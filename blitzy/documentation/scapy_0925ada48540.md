...
This analysis is performed against **Scapy 2026.04.09** (the development HEAD of the
repository). All line numbers are verified against this version.
...
`show()` output of the dissected HTTP packet (None-valued fields omitted for brevity):
...
do_dissect_payload(self, s) called with payload bytes s
  ...
  ├── except Exception:
  │     if conf.debug_dissector:                     # Truthy: log then re-raise
  │     │   log_runtime.error("...")
  │     │   if cls is not None:
  │     │       raise
  │     # Falsy (default): silent fallback
  │     p = conf.raw_layer(s, _internal=1, _underlayer=self)
...
**Thinking:** The two-mode design (`conf.debug_dissector` truthy vs. falsy) gives users
control over error visibility:

| `conf.debug_dissector` Value | Behavior | Use Case |
|---|---|---|
| `False` (**default**) | Silent fallback to `Raw` | Production packet capture, where resilience matters more than diagnostics |
| Truthy (e.g., `True`, `1`) | Log error message, then re-raise if `cls is not None` | Development of new dissectors, where you need to see what went wrong |
...
entire capture session. By defaulting to `False`, Scapy prioritizes data preservation over
...
> **Note:** A separate `conf.debug_dissector >= 2` check exists in `scapy/sendrecv.py:1245`
> within the `sniff()` pipeline's socket error handling. That check is scoped to
> `sendrecv.py`'s socket-level exception recovery, NOT to `do_dissect_payload()`. The
> dissection error handler in `packet.py:1036-1046` only checks truthiness.
...
| 1266-1325 | `getlayer(self, cls, nb=1)` | Traverses layer chain to find Nth occurrence of a class | §5.5 |
...
| 1036-1046 | `except Exception:` block | Error handling with two debug_dissector modes (truthy/falsy) | §3.3, §4.4 |
...
| Line(s) | Symbol | Purpose | Referenced In |
|---|---|---|---|
| 521-550 | `class IP(Packet, IPTools)` | IPv4 packet with 12+ fields (`IPTools` mixin adds IP-specific utilities) | §2.2, §5.2 |
...
| **`conf.debug_dissector`** | Controls error handling mode in `do_dissect_payload()`: `False` (default)=silent fallback to `Raw`, truthy=log error and re-raise if `cls is not None`. A separate `>= 2` check exists only in `scapy/sendrecv.py:1245`. | `scapy/packet.py:1036-1046` |
...
|---|---|---|---|
| 0 | `{'type': 122}` | `LLC` | `scapy/layers/l2.py:687` |
| 1 | `{'type': 34928}` | `LLC` | `scapy/layers/l2.py:688` |
| 2 | `{'type': 33024}` | `Dot1Q` | `scapy/layers/l2.py:689` |
| 3 | `{'type': 34984}` | `Dot1AD` | `scapy/layers/l2.py:690` |
| 4 | `{'type': 1}` | `Ether` | `scapy/layers/l2.py:694` |
| 5 | `{'type': 2054}` | `ARP` | `scapy/layers/l2.py:695` |
| 6 | `{'type': 34980}` | `EtherCat` | Loaded EtherCat layer |
| 7 | `{'type': 2048}` | `IP` | `scapy/layers/inet.py:1101` |
| 8 | `{'type': 34525}` | `IPv6` | `scapy/layers/inet6.py:4083` |
| 9 | `{'type': 34958}` | `EAPOL` | Loaded EAP layer |
| 10 | `{'type': 34915}` | `PPPoED` | Loaded PPP layer |
| 11 | `{'type': 34916}` | `PPPoE` | Loaded PPP layer |
| 12 | `{'type': 32801}` | `PPP_IPCP` | Loaded PPP layer |
| 13 | `{'type': 32851}` | `PPP_ECP` | Loaded PPP layer |
| 14 | `{'type': 35033}` | `LLTD` | Loaded LLTD layer |
| 15 | `{'type': 41197}` | `SixLoWPAN` | Loaded 6LoWPAN layer |
...
0-5). Then EtherCat (index 6), `inet` (index 7), `inet6` (index 8), and so on through the
remaining loaded layers. The `IP` binding at index 7 is the one that drives IPv4 dissection
for the vast majority of Ethernet traffic. This data is consistent with the runtime evidence
in Section 1.2 and the binding table in Section 1.5.
...
| # | Field Conditions | Next Class | Registration Source |
|---|---|---|---|
| 0 | `{'frag': 0, 'proto': 4}` | `IP` | `scapy/layers/inet.py:1111` |
| 1 | `{'frag': 0, 'proto': 1}` | `ICMP` | `scapy/layers/inet.py:1112` |
...
| 6 | `{'proto': 51}` | `AH` | `scapy/layers/ipsec.py` |
| 7 | `{'proto': 50}` | `ESP` | `scapy/layers/ipsec.py` |
| 8 | `{'proto': 132}` | `SCTP` | `scapy/layers/sctp.py` |
| 9 | `{'proto': 112}` | `VRRP` | Loaded VRRP layer |
| 10 | `{'proto': 112}` | `VRRPv3` | Loaded VRRP layer |

**Analysis of the `frag=0` requirement:** All five `inet.py` entries (0-4: IP, ICMP, TCP, UDP,
GRE) require `frag=0` in addition to the `proto` field. This is because the source code at
`scapy/layers/inet.py:1111-1115` registers all five with `frag=0`:
...
When an IP packet is fragmented (`frag > 0`), the payload bytes represent a fragment of the
original packet, not the start of a new protocol header. Dissecting a fragment as TCP, ICMP,
or any other protocol would produce garbled results. By requiring `frag=0`, Scapy ensures
that only the first fragment (or unfragmented packets) trigger protocol dissection.

Entries 5-10 (from `inet6.py`, `ipsec.py`, `sctp.py`, and VRRP layers) do NOT require
`frag=0` — they only match on `proto`. This data is consistent with the runtime evidence in
Section 1.2 and the binding table in Section 1.5.
...
| 8 | `{'proto': 34958}` | `EAPOL` | Loaded EAP layer |
...
Entry 2 uses `type` instead of `proto`. Since GRE's `fields_desc` names this field `proto`
(not `type`), the `type=0x88a8` condition in `payload_guess` does **not** match during
dissection — `getfieldval('type')` cascades through to `NoPayload.getfieldval('type')`
(`scapy/packet.py:1783`) which raises `AttributeError`, silently caught by the `try/except`
in `guess_payload_class()` (`scapy/packet.py:1077`), causing this binding entry to be skipped.
This binding only functions for top-down (packet building) purposes via `_overload_fields`.
This appears to be a latent issue in the Scapy source code at `scapy/layers/l2.py:704`.

---

*Document generated from analysis of Scapy 2026.04.09 source code (development HEAD). All line numbers verified
