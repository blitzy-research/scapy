...
> **Deprecated legacy wrapper:** The function `DNSgetstr()` (Source: `scapy/layers/dns.py:175–181`) is a deprecated wrapper around `dns_get_str()`. It passes all arguments through to `dns_get_str()` but returns only the first 3 elements of the 4-tuple (via slicing `[:-1]`), omitting the compression flag. Calling `DNSgetstr()` emits a `DeprecationWarning` advising callers to use `dns_get_str()` instead. This wrapper exists for backward compatibility with code written before the 4-element return value was introduced.
...
The function enters a `while True` loop. Each iteration begins with two preliminary steps (bounds checking and byte reading), then branches into one of three code paths based on the value of the current byte:

#### Preliminary Step A: Out-of-Bounds Check (lines 96–100)
...
#### Preliminary Step B: Reading the Current Byte (lines 101–102)
...
#### Path 1: Compression Pointer — `cur & 0xc0` is truthy (lines 103–131)
...
#### Path 2: Normal Label — `cur > 0` (lines 132–135)
...
#### Path 3: Terminator — `cur == 0` (lines 136–137)
...
| `DNSRROPT` | `dns.py:571` | EDNS0 OPT resource record — needs cross-boundary for its `rrname` `DNSStrField` |
| `_DNSRRdummy` | `dns.py:785` | Internal base class for RR types with `post_build()` rdlen recalculation (e.g., `DNSRRMX`, `DNSRRSOA`, `DNSRRSRV`) — needs cross-boundary for rdata domain name fields |
...
Understanding the full build→compress→transmit→dissect→decompress lifecycle is architecturally significant because DNS name compression is not an isolated feature — it is a cross-cutting concern that touches every stage of a DNS packet's life. On the building side, encoding (`dns_encode()`) must produce well-formed wire-format labels *before* compression (`dns_compress()`) can identify shared suffixes and replace them with pointers. On the parsing side, the dissection pipeline must carefully thread the full original packet bytes (`_orig_s`) through every record and field so that `dns_get_str()` can resolve pointers that reference any position in the message. A developer who understands only decompression but not compression (or vice versa) will miss critical interactions — for example, the `check_built` guard in `dns_encode()` exists specifically to prevent double-encoding during the compression phase, and the `compressed` flag on `DNSStrField` exists to trigger `rdlen` recomputation after decompression. This section ties all those pieces together into a single coherent picture.
...
6. **`DNS.pre_dissect()`** (Source: `dns.py:518–538`): Validates the packet structure (especially for DNS over TCP, checking length field consistency and that the DNS message length is not too small or mismatched).
